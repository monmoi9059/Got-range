    ...
    window.buyOrEquipSkin = function() {
        ...
        const applySkin = () => {
            // Check for animal change
            const oldSkinId = playerData.currentSkin;
            const oldSkinObj = SKINS_DB.find(s => s.id === oldSkinId);
            if (oldSkinObj && skin.animal !== oldSkinObj.animal) {
                // Reset items
                playerData.currentHat = 'hat_none';
                playerData.currentClothing = 'clothes_none';
                playerData.currentPants = 'pants_none';
            ...
            playerData.currentSkin = skin.id;
        };

        if (isUnlocked) {
            applySkin();
            checkAchievements('skin');
        }
        else if (playerData.tacos >= skin.cost) {
            playerData.tacos -= skin.cost;
