...
// --- HAIRSTYLE RENDERER V3.0 (Patched) ---

function adjustColor(color, amount) {
    if (!color) return '#000000';
    // Fix: Expand 3-digit hex if needed
    if (color.length === 4) {
...
function adjustColorAmount(color, amount) { return adjustColor(color, amount); }
...
function drawHairstyle(ctx, p, headY, headRadius, s, skinObj) {
    ...
    if (style.includes('braid') || style.includes('cornrow') || style.includes('dread')) {
        ...
        // --- SCALP FILL (Fix for skin showing) ---
        if (!isCornrow) {
            ctx.fillStyle = adjustColor(color, -20);
            ctx.beginPath();
            ctx.arc(p.x, headY, headRadius * 0.95, Math.PI, 0);
            ctx.fill();
        }
    ...
    ctx.fillStyle = color;
    ctx.beginPath();
    // EXTENDED START POINT (Fix for skin showing)
    ctx.moveTo(p.x - sideW, headY + 3*s);

    // Top Arch
    if (style === 'side_part') {
        ctx.bezierCurveTo(p.x - sideW, headY - topH*0.8, p.x + sideW, headY - topH*1.2, p.x + sideW, headY + 3*s);
    } else {
        ctx.bezierCurveTo(p.x - sideW, headY - topH, p.x + sideW, headY - topH, p.x + sideW, headY + 3*s);
    }
