    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (anchors && anchors.shoulders && anchors.hips) {
            ...
            if (fitType === 'baggy') {
                ...
                // Detailed V-Taper removed per request (caused unwanted 'ribs' artifact)

                // Simple shape for clothes/low graphics (and now High Graphics Human too)
                points = [
        ...
        if (isFurry) {
            ...
            if (options.isJersey) {
                 ...
                 if (options.animal === 'human') {
                     // Human: Flat skin, no texture pattern to avoid artifacts
                     fillStyle = color;
                 } else {
                     ...
                     // Align pattern to body center (Fix repeating seam artifact)
                     if (fillStyle.setTransform && typeof DOMMatrix !== 'undefined') {
                         const matrix = new DOMMatrix();
                         // Center the gradient on the body (cx, midY)
                         // Gradient center is at size/2
                         // We want (size/2, size/2) -> (cx, topY + h/2)
                         const halfSize = GRAD_CANVAS_SIZE / 2;
                         matrix.translateSelf(cx - halfSize, topY + h/2 - halfSize);
                         fillStyle.setTransform(matrix);
                     }
                 }
            }
