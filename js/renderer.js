var BallRenderer = {
    ...
    draw3D: function(ctx, ball, phys) {
        // High Quality 3D Rendering (Remastered)
        ...
        // Pseudo-random noise function based on face index
        var getNoise = function(idx) {
            var x = Math.sin(idx * 12.9898) * 43758.5453;
    ...
    drawPattern3D: function(ctx, ball, phys) {
        ...
        var projectPoint = function(p) {
            ...
            if ((r.x*vx + r.y*vy + r.z*vz) <= 0) return null;
        ...
        if (type === 'basketball') {
            ...
            var points = Geometry.basketball;
            var segLen = 65;
            ...
            var color = (type === 'baseball') ? '#FF0000' : '#FFF';
            ...
            for(var i=0; i<Geometry.dimples.length; i++) {
                var d = Geometry.dimples[i];
                var proj = projectPoint(d);
                if (proj) {
                    var r = 25 * s * 0.05;
                    var gr = ctx.createLinearGradient(proj.x - r, proj.y - r, proj.x + r, proj.y + r);
                    gr.addColorStop(0, 'rgba(0,0,0,0.3)');
                    gr.addColorStop(1, 'rgba(255,255,255,0.3)');
    ...
    drawPattern: function(ctx, r, rot, ball) {
        ...
        if (type === 'basketball') {
            ...
            for(var i=0; i<Geometry.basketball.length; i++) {
                 if (i % 65 === 0) { ctx.stroke(); ctx.beginPath(); first=true; }
