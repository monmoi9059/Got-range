var BallRenderer = {
    ...
    function drawLaporteBridge(ctx, x, y, scale) {
        ctx.save();
        ctx.translate(x, y);
        ...
        // Piers (Supports in water) - Drawn first (behind)
        ctx.fillStyle = '#666';
        ctx.fillRect(-85, 10, 20, 100); // Left Pier
        ctx.fillRect(65, 10, 20, 100);  // Right Pier
    ...
    function drawQuebecBridge(ctx, x, y, scale) {
        ctx.save();
        ctx.translate(x, y);
        ctx.scale(scale, scale);
        ...
        // Piers
        ctx.fillStyle = '#666';
        ctx.fillRect(-85, 10, 10, 80);
        ctx.fillRect(-5, 10, 10, 80);
        ctx.fillRect(75, 10, 10, 80);
        ...
        ctx.fillStyle = '#444'; ctx.fillRect(-150, 0, 300, 5);
        ctx.restore();
    }
    ...
    function drawRiver(ctx, horizonY, vpW) {
         ...
         ctx.fillStyle = grad;
         ctx.fillRect(0, riverY, vpW, riverH);

         const camX = (g_camCache && g_camCache.x) ? g_camCache.x : 0;
         const camY = (g_camCache && g_camCache.y) ? g_camCache.y : 0;

         // Match mountain parallax roughly (between 0.02 and 0.1)
         const parallaxFactor = 0.03;
         const parallax = (camX + camY) * parallaxFactor;

         // Position bridges higher (horizonY - 20) so they sit over the water
         const bridgeBaseY = riverY - 10;
         const bridgeX = 150 - parallax;

         drawLaporteBridge(ctx, bridgeX, bridgeBaseY, 0.6);
         drawQuebecBridge(ctx, bridgeX - 250, bridgeBaseY, 0.6);
