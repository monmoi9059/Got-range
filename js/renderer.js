...
var BallRenderer = {
    ...
    const drawFuzzyCircle = (cx, cy, r, c, seed = 50, scale = 1.0, furry = true, applyShading = true) => {
        if (playerData.graphics === 'HIGH') {
            const rimColor = getCelColor(c, 'rim');
            const shadowColor = getCelColor(c, 'shadow');
            const baseColor = c;

            // Volumetric Gradient (Top-Left Light Source)
            // Highlight offset to top-left (-0.3r, -0.3r)
            const grad = ctx.createRadialGradient(cx - r*0.3, cy - r*0.3, 0, cx, cy, r);
            grad.addColorStop(0.0, baseColor);   // Highlight/Light
            grad.addColorStop(0.5, baseColor);   // Mid-tone
            grad.addColorStop(1.0, shadowColor); // Shadow Edge
    ...
    function drawContinuousLimb(p1, p2, p3, w1, w2, w3, color, scale, isFurry, seed = 1, options = {}) {
        ...
        if (isFurry) {
            if (playerData.graphics === 'HIGH') {
                 ...
                 // Gradient: Light (Left) -> Shadow (Right)
                 const grad = ctx.createLinearGradient(p1L.x, p1L.y, p1R.x, p1R.y);
                 grad.addColorStop(0.0, baseColor);    // Light Side
                 grad.addColorStop(0.4, baseColor);    // Mid
                 grad.addColorStop(1.0, shadowColor);  // Shadow Side
    ...
    function drawFuzzyLimb(x1, y1, x2, y2, width, color, scale, isFurry, seed = 1, endWidth = null) {
        ...
        if (playerData.graphics === 'HIGH') {
            ...
            // Gradient: Light (Left/Top) -> Shadow (Right/Bottom)
            const grad = ctx.createLinearGradient(p4.x, p4.y, p1.x, p1.y);
            grad.addColorStop(0.0, baseColor);
            grad.addColorStop(0.4, baseColor);
            grad.addColorStop(1.0, shadowColor);
    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (isFurry) {
            ...
            if (playerData.graphics === 'HIGH') {
                // VOLUMETRIC BACK VIEW - "More Details and 3D"
                // Light Source: Top Left (Studio) instead of Backlit
                const shadowColor = getCelColor(color, 'shadow');
                const baseColor = color;

                // Gradient: Left (Light) -> Right (Shadow)
                const grad = ctx.createLinearGradient(cx - w, 0, cx + w, 0);
                grad.addColorStop(0.0, baseColor);    // Light Side
                grad.addColorStop(0.4, baseColor);    // Mid
                grad.addColorStop(0.8, shadowColor);  // Shadow Side
                grad.addColorStop(1.0, shadowColor);  // Core Shadow
    ...
    function drawLimb(x1, y1, x2, y2, width, color) {
        ...
        if (playerData.graphics === 'HIGH') {
            // VOLUMETRIC SHADING (Studio Light)
            const shadowColor = getCelColor(color, 'shadow');
            const baseColor = color;

            // Gradient Top -> Bottom (Light -> Shadow)
    ...
    function drawJoint(x, y, radius, color, isMechanical) {
        ...
        if (playerData.graphics === 'HIGH') {
            ...
            // Radial Gradient (Backlit Sphere)
            const grad = ctx.createRadialGradient(x, y, 0, x, y, radius);
            grad.addColorStop(0.0, shadowColor);
            grad.addColorStop(0.6, baseColor);
            grad.addColorStop(1.0, rimColor);
