...
var BallRenderer = {
    ...
    function getCelColor(baseColor, type) {
        if (!baseColor || !baseColor.startsWith('#')) return baseColor;
        let hsl = hexToHsl(baseColor);
        if (type === 'shadow') {
            hsl.l = Math.max(0, hsl.l - 20); // Darker Shadow for more volume
            hsl.s = Math.min(100, hsl.s + 10);
            hsl.h = (hsl.h + 5) % 360;
        } else if (type === 'rim') {
            // REMOVE HIGHLIGHT BOOST - User Request "remove the highlight around everything"
            // Rim is now just Base Color or very slightly shifted, not brighter
            hsl.l = Math.min(100, hsl.l + 2); // Barely noticeable if at all
    ...
    function drawBacklitShape(ctx, points, color, lineWidth, s) {
        ...
        if (playerData.graphics === 'HIGH') {
            drawColor = getCelColor(color, 'shadow');
            rimColor = getCelColor(color, 'rim');
        }

        // 1. Rim Light (Stroke)
        if (rimColor) {
            ctx.strokeStyle = rimColor;
            ctx.lineWidth = (lineWidth > 0 ? lineWidth : 2*s) * 1.5;
            ctx.lineCap = 'round';
            ctx.lineJoin = 'round';
            ctx.beginPath();
            ctx.moveTo(points[0].x, points[0].y);
            for(let i=1; i<points.length; i++) ctx.lineTo(points[i].x, points[i].y);
            // Close if needed? Usually shapes are closed.
    ...
    const drawFuzzyCircle = (cx, cy, r, c, seed = 50, scale = 1.0, furry = true, applyShading = true) => {
        if (playerData.graphics === 'HIGH') {
            const shadowColor = getCelColor(c, 'shadow');
            const baseColor = c;
            const highlightColor = getCelColor(c, 'highlight');

            // Volumetric Gradient (Light from Behind/Camera)
            // Centered Highlight, Shadow Edges
            const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
            grad.addColorStop(0.0, highlightColor);
            ...
            if (furry) {
                ...
                // Gradient Fill (Pass Gradient Object as Color)
                drawFuzzyPath(g_circlePoints, grad, scale, true, seed);

                // Optional: No Rim Stroke as requested ("remove highlight")
                // Just fuzzy edge defined by shadow falloff
            } else {
                // Smooth Cel Circle
    ...
    function drawContinuousLimb(p1, p2, p3, w1, w2, w3, color, scale, isFurry, seed = 1, options = {}) {
        ...
        if (isFurry) {
            if (playerData.graphics === 'HIGH') {
                 const shadowColor = getCelColor(color, 'shadow');
                 const baseColor = color;
                 const highlightColor = getCelColor(color, 'highlight');

                 // Gradient: Symmetrical Cylindrical (Shadow -> Light -> Shadow)
                 const grad = ctx.createLinearGradient(p1L.x, p1L.y, p1R.x, p1R.y);
                 grad.addColorStop(0.0, shadowColor);
                 ...
                 // Fill
                 drawFuzzyPath(points, grad, scale, true, seed);
                 // No Rim Stroke
            } else {
        ...
        // Apply Cylindrical Shading (Gradients) - LOW/MED ONLY
        if (playerData.graphics !== 'HIGH') {
            // Upper Arm Shading
            ...
            // Draw rect covering upper segment
            ctx.beginPath();
    ...
    function drawFuzzyLimb(x1, y1, x2, y2, width, color, scale, isFurry, seed = 1, endWidth = null) {
        ...
        if (playerData.graphics === 'HIGH') {
            ...
            // Gradient: Symmetrical Cylindrical (Shadow -> Light -> Shadow)
            // p1 is Left, p4 is Right (roughly, depending on winding)
            ...
            // Gradient Fill
            drawFuzzyPath(g_limbPoints, grad, scale, true, seed);
        } else {
    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (isFurry) {
            ...
            if (playerData.graphics === 'HIGH') {
                // VOLUMETRIC BACK VIEW - Light from Behind (Camera)
                const shadowColor = getCelColor(color, 'shadow');
                const baseColor = color;
                const highlightColor = getCelColor(color, 'highlight');

                // Gradient: Edge (Shadow) -> Center (Light) -> Edge (Shadow)
                ...
                // ADD DETAILS: Subtle Muscle Hints (Spine & Lats)
                if (!options.isJersey) {
                    ctx.save();
                    ctx.clip(); // Clip to body

                    // Spine Groove (Shadow down middle)
                    const spineGrad = ctx.createLinearGradient(cx - 5*scale, 0, cx + 5*scale, 0);
                    spineGrad.addColorStop(0, 'rgba(0,0,0,0)');
                    spineGrad.addColorStop(0.5, 'rgba(0,0,0,0.15)'); // Subtle indentation
                    spineGrad.addColorStop(1, 'rgba(0,0,0,0)');
                    ctx.fillStyle = spineGrad;
                    ctx.fillRect(cx - 10*scale, topY, 20*scale, h);

                    // Scapula / Shoulder Blades (Subtle blobs)
                    ctx.fillStyle = 'rgba(0,0,0,0.08)'; // Very subtle shadow
                    // Left Blade
                    ctx.beginPath();
                    ctx.ellipse(cx - w*0.4, topY + h*0.25, w*0.15, h*0.1, -0.3, 0, Math.PI*2);
                    ctx.fill();
                    // Right Blade
                    ctx.beginPath();
                    ctx.ellipse(cx + w*0.4, topY + h*0.25, w*0.15, h*0.1, 0.3, 0, Math.PI*2);
            ...
            // Muscle Definition removed for smooth look
    ...
    function drawLimb(x1, y1, x2, y2, width, color) {
        ...
        if (playerData.graphics === 'HIGH') {
            // VOLUMETRIC SHADING (Top-Right Light Source)
            // Rotated Space: Top (-w/2) is Left side relative to limb axis?
            // Actually drawLimb uses local Y from -width/2 to +width/2.
            // If limb is vertical, -width/2 is Left.
            // We want Light on Right (+width/2).
            const shadowColor = getCelColor(color, 'shadow');
            const baseColor = color;

            // Gradient: Shadow -> Light
            const grad = ctx.createLinearGradient(0, -width/2, 0, width/2);
            grad.addColorStop(0.0, shadowColor); // Left (Shadow)
            grad.addColorStop(0.6, baseColor);   // Mid
            grad.addColorStop(1.0, baseColor);   // Right (Light)
    ...
    function drawJoint(x, y, radius, color, isMechanical) {
        ...
        // Shading
        if (playerData.graphics === 'HIGH') {
            const rimColor = getCelColor(color, 'rim');
            const shadowColor = getCelColor(color, 'shadow');
            const baseColor = color;

            // Radial Gradient (Top-Right Light)
            // Offset highlight to (+, -)
            const grad = ctx.createRadialGradient(x + radius*0.3, y - radius*0.3, 0, x, y, radius);
            grad.addColorStop(0.0, baseColor); // Highlight
            grad.addColorStop(0.5, baseColor);
            grad.addColorStop(1.0, shadowColor); // Shadow
    ...
    function drawRealisticShadow(obj, type) {
        ...
        // Soft Oval Shadow (Wind Waker / High Graphics Style)
        ...
        // Base radius
        let r = (type === 'player') ? 25 : 10;
        r *= obj.scale;

        // Z effect: Shadow shrinks slightly and fades
        let zFactor = Math.max(0, 1 - (z / 1000));
        r *= (0.8 + 0.2 * zFactor);

        let alpha = 0.4 * zFactor;
    ...
    function drawMuscleLimb(x1, y1, x2, y2, width, color, type, s, hasTattoos) {
        // Redirect legacy muscle calls to the new smooth limb renderer
        drawLimb(x1, y1, x2, y2, width, color);
    }
    ...
    function drawShorts(cx, waistY, w, h, scale, skinObj) {
        ...
        let fillStyle;
        if (playerData.graphics === 'HIGH') {
             fillStyle = getCelColor(color, 'shadow');
        } else {
    ...
    function drawHairstyle(ctx, p, headY, headRadius, s, skinObj) {
        ...
        // User Request: Make base size 5% bigger and raise them on the skull by 5%
        // User Request: Another 5% (Total 10%)
        const modRadius = headRadius * hairScale * 1.10;
        headY -= headRadius * 0.10;
    ...
    function drawRealisticHuman(p, s, skinObj) {
        ...
        if (skinObj.headType && skinObj.headType !== 'human') {
            ...
            if (playerData.graphics === 'HIGH') {
                const rimColor = getCelColor(skinTone, 'rim');
                const shadowColor = getCelColor(skinTone, 'shadow');
                ctx.strokeStyle = rimColor;
                ctx.lineWidth = 3 * s;
                ctx.stroke();
                ctx.fillStyle = shadowColor;
    ...
    function drawBackground(vpX, vpY, vpW, vpH, alpha) {
        ...
        if (!bgCache || bgCache.distanceLevel !== distanceLevel || bgCache.mode !== currentGameMode) {
            ...
            // Current Floor
            if (playerData.graphics === 'HIGH') {
                 // High Graphics: Baked canvas with Sun Glare (Wind Waker style)
                 const fCv = document.createElement('canvas');
                 fCv.width = Math.ceil(vpW);
                 fCv.height = Math.ceil(vpH - horizonY);
                 const fCtx = fCv.getContext('2d');

                 // Base
                 const grad = fCtx.createLinearGradient(0, 0, 0, fCv.height);
                 grad.addColorStop(0, court.ground1);
                 grad.addColorStop(1, court.ground2);
                 fCtx.fillStyle = grad;
                 fCtx.fillRect(0, 0, fCv.width, fCv.height);

                 // Sun Glare (Horizon Sheen)
                 const glare = fCtx.createLinearGradient(0, 0, 0, fCv.height * 0.4);
                 glare.addColorStop(0, 'rgba(255, 255, 255, 0.4)');
                 glare.addColorStop(1, 'rgba(255, 255, 255, 0)');
                 fCtx.fillStyle = glare;
                 fCtx.fillRect(0, 0, fCv.width, fCv.height * 0.4);
