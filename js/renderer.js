    ...
    function drawDecor(p, type, variant, seed) {
        ...
        if(type === 'grass') {
            ...
            if (seed > 0.6) {
             ...
             let ageScale = 0.075; // Kitten (Tiny - 1/8th of adult)
             let furColor = '#D2B48C'; // Tan/Orange
             let bellyColor = '#F5F5DC'; // Cream

             // Growth Stages
             if (makes >= 500) {
                 ageScale = 1.1; // Giant/Old (Reduced from 1.6)
                 furColor = '#A9A9A9'; // Grey
             } else if (makes >= 200) {
                 ageScale = 0.7; // Fat/Adult (Reduced from 1.0)
                 furColor = '#8B4513'; // Darker Brown
             } else if (makes >= 50) {
                 ageScale = 0.3; // Adolescent (Reduced from 0.4)
