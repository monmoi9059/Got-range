var BallRenderer = {
    ...
    // --- HAIR HELPERS ---
    function drawHairStrands(ctx, p, headY, radius, s, color, count, length, curl, spread, baseSeed) {
        ctx.lineCap = 'round';
        ctx.strokeStyle = color;
        ctx.lineWidth = 3 * s;
        for(let i=0; i<count; i++) {
            const seed = baseSeed + i * 13;
            // Spread across the top hemisphere
            const angle = Math.PI + (seededRandom(seed) * Math.PI); // PI to 2PI (Top arc)
            // Z-depth simulation for starting point
            const zOff = (seededRandom(seed + 1) - 0.5) * radius * 0.8;

            // Start Point on Scalp
            const sx = p.x + Math.cos(angle) * (radius * 0.9);
            const sy = (headY - 2*s) + Math.sin(angle) * (radius * 0.9) + zOff * 0.5;

            // Variation
            const l = length * (0.8 + 0.4 * seededRandom(seed + 2));

            ctx.beginPath();
            ctx.moveTo(sx, sy);

            // Bezier for Curl
            const cp1x = sx + (seededRandom(seed + 3) - 0.5) * spread;
            const cp1y = sy + l * 0.3;
            const cp2x = sx + (seededRandom(seed + 4) - 0.5) * spread;
            const cp2y = sy + l * 0.6;
            const ex = sx + (seededRandom(seed + 5) - 0.5) * spread * 0.5; // Ends mostly down
            const ey = sy + l;
    ...
    function drawHairTexture(ctx, p, headY, radius, s, color, density, seed, type) {
        ctx.fillStyle = adjustColor(color, 20); // Highlights
        for(let i=0; i<density; i++) {
            const rndA = seededRandom(seed + i * 10) * Math.PI * 2;
            const rndR = seededRandom(seed + i * 20) * radius * 0.85;
            const cx = p.x + Math.cos(rndA)*rndR;
            const cy = (headY - 2*s) + Math.sin(rndA)*rndR;

            // Clip to circle
            if (Math.sqrt((cx-p.x)**2 + (cy-(headY-2*s))**2) > radius) continue;

            if (type === 'stubble') {
                ctx.beginPath(); ctx.arc(cx, cy, 0.8*s, 0, Math.PI*2); ctx.fill();
            } else if (type === 'curls') {
    ...
    function drawHairstyle(ctx, p, headY, headRadius, s, skinObj) {
        ...
        // Lift hair center slightly (20% of radius) to sit on top of skull
        ...
        // --- BATCH 1: SHORT / FADE / BALD ---

        if (style === 'bald' || style === 'beard') { // Beard style is just bald on top usually
             // Shiny scalp
        ...
        if (style === 'short' || style === 'buzz_cut' || style === 'crew_cut') {
             const r = modRadius * 1.02;
             // Soft gradient cap
             const grad = ctx.createRadialGradient(p.x, headY - 2*s, r*0.5, p.x, headY - 2*s, r);
             grad.addColorStop(0, hairColor);
             grad.addColorStop(1, adjustColor(hairColor, -10)); // Slightly darker edge

             ctx.fillStyle = grad;
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, r, 0, Math.PI*2); ctx.fill();

             // Stubble Texture
             drawHairTexture(ctx, p, headY, r, s, hairColor, 40, baseSeed, 'stubble');

             // Sharp Hairline (Rectangle clip at forehead if front view, but this is BACK view)
             // For back view, short hair has a neck taper.
             // Squared Neckline
             ctx.fillStyle = skinObj.skinTone || '#8d5524';
             ctx.beginPath();
             ctx.moveTo(p.x - r*0.6, headY + r*0.8);
             ctx.lineTo(p.x + r*0.6, headY + r*0.8);
             ctx.lineTo(p.x + r*0.6, headY + r*1.2);
             ctx.lineTo(p.x - r*0.6, headY + r*1.2);
             ctx.fill(); // "Cut" the bottom

             // Re-draw neck connection (Skin)
             return;
        }

        if (style === 'fade' || style === 'caesar' || style === 'french_crop' || style === 'luka_fade' || style === 'comb_over') {
             const r = modRadius * 1.02;

             // Top (Darker/Fuller)
             ctx.fillStyle = hairColor;
             ctx.beginPath(); ctx.arc(p.x, headY - 4*s, r, Math.PI, 0); // Top half
             ctx.fill();

             // Fade Gradient (Sides)
             const fadeGrad = ctx.createLinearGradient(0, headY - 5*s, 0, headY + r);
             fadeGrad.addColorStop(0, hairColor);
             fadeGrad.addColorStop(0.6, 'rgba(0,0,0,0)'); // Fade out down neck

             ctx.fillStyle = fadeGrad;
             ctx.beginPath();
             ctx.rect(p.x - r, headY - 5*s, r*2, r*1.5);
             ctx.fill();

             drawHairTexture(ctx, p, headY, r, s, hairColor, 30, baseSeed, 'stubble');
             return;
        }
        ...
        if (style === 'undercut') {
             ...
             ctx.fillStyle = hairColor;
             ctx.beginPath();
             ctx.ellipse(p.x, headY - r*0.4, r*0.9, r*0.6, 0, Math.PI, 0); // Top dome
             ctx.fill();
             // Sides are skin (Bald) - Handled by head drawing
             return;
        }

        // --- BATCH 2: AFRO / TEXTURED / BRAIDS ---

        if (style === 'afro' || style === 'short_afro') {
             let sizeMult = (style === 'short_afro') ? 1.15 : (skinObj.afroSize || 1.3);
             ...
             // Fuzzy Edge
             drawFuzzyCircle(p.x, headY - 2*s, r, hairColor, baseSeed, s, true, false); // Base fuzz

             // Volume Texture
             drawHairTexture(ctx, p, headY, r, s, hairColor, 20, baseSeed, 'curls');
             return;
        }

        if (style === 'curly_high' || style === 'shag' || style === 'messy_bedhead') {
             const r = modRadius * 1.2;
             // Messy shape
             ctx.fillStyle = hairColor;
             drawFuzzyCircle(p.x, headY - 2*s, r, hairColor, baseSeed, s, true, true);
             // Extra lumps
             drawFuzzyCircle(p.x - r*0.4, headY - r*0.4, r*0.5, hairColor, baseSeed+1, s, true, true);
             drawFuzzyCircle(p.x + r*0.4, headY - r*0.4, r*0.5, hairColor, baseSeed+2, s, true, true);
             drawFuzzyCircle(p.x, headY - r*0.6, r*0.6, hairColor, baseSeed+3, s, true, true);
             return;
        }

        if (style === 'cornrows') {
             // Iverson Style: Tight rows to scalp
             const r = modRadius * 1.02;
             // Scalp base
             ...
             for(let i=0; i<numRows; i++) {
                 ...
                 // Draw Arc following head curvature
                 ctx.beginPath();
                 const rowPoints = [];
                 for(let j=0; j<=10; j++) {
                     const yt = j/10; // 0 (front) to 1 (back/neck)
                     const yPos = (headY - r) + (yt * r * 2);

                     // Perspective Z-depth estimation
                     const zFactor = Math.sin(yt * Math.PI); // 0 at ends, 1 in middle? No.
                     // Simple 2D projection of sphere lines
                     // x^2 + y^2 + z^2 = r^2
                     // We fix X, vary Y.

                     // Naive linear for vector style looks cleaner
                     const px = p.x + xOff * (1 - yt*0.2); // Slight taper to neck
                     const py = (headY - r*0.8) + yt * (r * 1.8);

                     if (j===0) ctx.moveTo(px, py);
                     else ctx.lineTo(px, py);
                 }
                 ctx.strokeStyle = 'rgba(0,0,0,0.5)'; ctx.lineWidth = 4*s; ctx.stroke();
                 ctx.strokeStyle = hairColor; ctx.lineWidth = 3*s; ctx.stroke();
             }
             return;
        }

        if (style === 'braids_back') {
             // Kawhi Style: Cornrows + Hanging Braids
             // Draw Cornrows first (base)
             const r = modRadius * 1.02;
             ctx.fillStyle = adjustColor(hairColor, -20);
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, r, 0, Math.PI*2); ctx.fill();
             ...
             for(let i=0; i<7; i++) {
                 ...
                 ctx.beginPath(); ctx.moveTo(p.x + xOff, headY - r); ctx.lineTo(p.x + xOff, headY + r); ctx.stroke();
             }

             // Hanging Braids
             drawHairStrands(ctx, p, headY, r, s, hairColor, 12, 15*s, 0, 15*s, baseSeed);
             return;
        }

        if (style === 'dreads_short' || style === 'dreads' || style === 'snakes' || style === 'medusa') {
             // Butler / Crowder
             const r = modRadius * 1.05;
             ctx.fillStyle = adjustColor(hairColor, -10);
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, r, 0, Math.PI*2); ctx.fill();

             // Wild hanging strands
             drawHairStrands(ctx, p, headY, r, s, hairColor, 20, 20*s, 1, 30*s, baseSeed);
             return;
        }

        if (style === 'short_curly' || style === 'waves' || style === 'perm') {
             // Tight curls texturing
             const r = modRadius * 1.05;
             const grad = ctx.createRadialGradient(p.x, headY - 2*s, r*0.5, p.x, headY - 2*s, r);
             grad.addColorStop(0, hairColor);
             ...
             drawHairTexture(ctx, p, headY, r, s, hairColor, 50, baseSeed, 'curls');
             return;
        }

        if (style === 'twisted_fade') {
             const r = modRadius * 1.1;
             ctx.fillStyle = hairColor;
             // Top Only
             ctx.beginPath(); ctx.arc(p.x, headY - 4*s, r, Math.PI, 0); ctx.fill();
             drawHairTexture(ctx, p, headY - 4*s, r, s, hairColor, 20, baseSeed, 'curls');
             // Skin Fade Sides
             return;
        }
        ...
        if (style === 'long' || style === 'long_straight' || style === 'curtains') {
             // Nash Flow
             const r = modRadius * 1.1;
             ctx.fillStyle = hairColor;

             // Top Cap
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, r, Math.PI, 0); ctx.fill();

             // Sides Flowing Down
             ctx.beginPath();
             ctx.moveTo(p.x - r, headY);
             ctx.quadraticCurveTo(p.x - r*1.2, headY + 20*s, p.x - r*0.5, headY + 25*s); // Left tip
             ctx.quadraticCurveTo(p.x, headY + 20*s, p.x + r*0.5, headY + 25*s); // Right tip
             ctx.quadraticCurveTo(p.x + r*1.2, headY + 20*s, p.x + r, headY);
             ctx.fill();
             return;
        }

        if (style === 'long_wavy' || style === 'dirk_shaggy') {
             // Wavy Flow
             const r = modRadius * 1.15;
             ctx.fillStyle = hairColor;

             // Draw as fuzzy strands packed
             drawHairStrands(ctx, p, headY, r, s, hairColor, 30, 25*s, 1, 20*s, baseSeed);
             // Fill gaps
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, r*0.9, 0, Math.PI*2); ctx.fill();
             return;
        }

        if (style === 'mullet_modern' || style === 'mullet') {
             // Business front (short), Party back (long)
             const r = modRadius * 1.05;
             // Short top
             ctx.fillStyle = hairColor;
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, r, Math.PI, 0); ctx.fill();

             // Long Back (Neck curtain)
             ctx.beginPath();
             ctx.moveTo(p.x - r*0.8, headY);
             ctx.lineTo(p.x - r*0.9, headY + 20*s);
             ctx.lineTo(p.x + r*0.9, headY + 20*s);
        ...
        if (style === 'pompadour' || style === 'slicked_back') {
             // The Anchor / Retro
             const r = modRadius * 1.1;
             const grad = ctx.createLinearGradient(0, headY - r, 0, headY);
             grad.addColorStop(0, adjustColor(hairColor, 30)); // Shine on top
             grad.addColorStop(1, hairColor);
             ctx.fillStyle = grad;

             ctx.beginPath();
             ctx.moveTo(p.x - r*0.8, headY + 5*s);
             ctx.quadraticCurveTo(p.x - r, headY - r*1.5, p.x, headY - r*1.5); // High top
             ctx.quadraticCurveTo(p.x + r, headY - r*1.5, p.x + r*0.8, headY + 5*s);
             ctx.fill();
             return;
        }
        ...
        if (style === 'flat_top') {
             ...
             ctx.lineTo(p.x + w*0.7, headY + 2*s);
             ctx.lineTo(p.x - w*0.7, headY + 2*s);
             ctx.fill();
             ...
             ctx.fillStyle = adjustColor(hairColor, 20);
             ctx.beginPath();
             ctx.ellipse(p.x, topY, w*0.8, 5*s, 0, 0, Math.PI*2);
             ctx.fill();
             return;
        }

        if (style === 'mohawk' || style === 'liberty_spikes' || style === 'spikes' || style === 'faux_hawk') {
             ctx.fillStyle = hairColor;
             const spikeH = (style === 'liberty_spikes') ? 40*s : 20*s;
             const w = (style === 'mohawk') ? 10*s : 30*s; // Mohawk narrow

             // Central strip
             ctx.beginPath();
             ctx.moveTo(p.x - 5*s, headY + 10*s);
             ctx.lineTo(p.x - 2*s, headY - 10*s - spikeH);
             ctx.lineTo(p.x + 2*s, headY - 10*s - spikeH);
             ctx.lineTo(p.x + 5*s, headY + 10*s);
             ctx.fill();
             ...
             if (style !== 'mohawk') {
                 for(let i=-2; i<=2; i++) {
                     ctx.beginPath();
                     ctx.moveTo(p.x + i*8*s - 4*s, headY);
                     ctx.lineTo(p.x + i*8*s, headY - spikeH);
                     ctx.lineTo(p.x + i*8*s + 4*s, headY);
                     ctx.fill();
                 }
             }
             return;
        ...
        if (style === 'bun_low' || style === 'top_knot' || style === 'man_bun' || style === 'samurai_bun' || style === 'dread_bun' || style === 'space_buns') {
             // Base cap
             ctx.fillStyle = hairColor;
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, headRadius * 1.02, 0, Math.PI*2); ctx.fill();

             const bunColor = adjustColor(hairColor, 10);
             ctx.fillStyle = bunColor;

             if (style === 'space_buns') {
                 drawFuzzyCircle(p.x - headRadius*0.8, headY - headRadius*0.8, 8*s, bunColor, baseSeed, s, true, true);
                 drawFuzzyCircle(p.x + headRadius*0.8, headY - headRadius*0.8, 8*s, bunColor, baseSeed+1, s, true, true);
             }
             else if (style === 'top_knot' || style === 'samurai_bun') {
                 drawFuzzyCircle(p.x, headY - headRadius*1.2, 8*s, bunColor, baseSeed, s, true, true);
             }
             else { // Low bun / dread bun
                 drawFuzzyCircle(p.x, headY + 2*s, 10*s, bunColor, baseSeed, s, true, true);
             }
             return;
        }

        if (style === 'ponytail_high' || style === 'ponytail_low') {
             ctx.fillStyle = hairColor;
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, headRadius * 1.02, 0, Math.PI*2); ctx.fill();

             // Tail
             ctx.beginPath();
             const startY = (style === 'ponytail_high') ? headY - 10*s : headY + 5*s;
             ctx.moveTo(p.x, startY);
             ctx.quadraticCurveTo(p.x + 10*s, startY + 15*s, p.x, startY + 30*s); // Swing
             ctx.lineWidth = 8*s;
             ctx.strokeStyle = hairColor;
             ctx.stroke();
             return;
        ...
        if (style === 'pigtails') {
             ...
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, headRadius * 1.02, 0, Math.PI*2); ctx.fill();
             // Left
             ctx.beginPath(); ctx.moveTo(p.x - headRadius*0.8, headY);
             ctx.quadraticCurveTo(p.x - headRadius*1.5, headY + 10*s, p.x - headRadius*1.8, headY + 20*s);
             ctx.lineWidth = 6*s; ctx.strokeStyle = hairColor; ctx.stroke();
             // Right
             ctx.beginPath(); ctx.moveTo(p.x + headRadius*0.8, headY);
             ctx.quadraticCurveTo(p.x + headRadius*1.5, headY + 10*s, p.x + headRadius*1.8, headY + 20*s);
             ctx.stroke();
        ...
        if (style === 'bob' || style === 'bowl_cut') {
             const r = modRadius * 1.1;
             ctx.fillStyle = hairColor;
             ctx.beginPath();
             ctx.arc(p.x, headY - 2*s, r, Math.PI, 0); // Top
             ctx.lineTo(p.x + r, headY + 10*s); // Straight down
             ctx.lineTo(p.x - r, headY + 10*s); // Straight bottom
        ...
        if (style === 'pixie') {
             const r = modRadius * 1.05;
             ctx.fillStyle = hairColor;
             drawFuzzyCircle(p.x, headY - 4*s, r, hairColor, baseSeed, s, true, true);
             return;
        }

        if (style === 'beehive') {
             // Marge Simpson style / Retro
             const w = headRadius * 1.2;
             const h = headRadius * 2.5;
             ctx.fillStyle = hairColor;
             ctx.beginPath();
             ctx.ellipse(p.x, headY - h*0.4, w*0.9, h*0.6, 0, 0, Math.PI*2);
             ctx.fill();
             return;
        }
        ...
        if (style === 'emo_fringe') {
             ...
             ctx.beginPath();
             ctx.moveTo(p.x + r, headY);
             ctx.lineTo(p.x - r*1.2, headY + 15*s); // Long fringe
        ...
        if (style === 'clown') {
             ctx.fillStyle = '#FF0000'; // Clown red
             drawFuzzyCircle(p.x - headRadius, headY, 10*s, '#FF0000', baseSeed, s, true, true);
             drawFuzzyCircle(p.x + headRadius, headY, 10*s, '#FF0000', baseSeed+1, s, true, true);
             // Bald center
             return;
        }
        ...
        if (style === 'monk') {
             ...
             ctx.strokeStyle = hairColor;
             ctx.lineWidth = 4*s;
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, headRadius, 0, Math.PI*2); ctx.stroke();
             return;
        }

        // --- ACCESSORY OVERLAYS (Integrated) ---
        // Hat style overrides handled via 'headAccessory' check in drawRealisticHuman,
        // but 'style === hat' exists in DB for fake_mario etc.
        if (style === 'hat') {
             const hatCol = skinObj.hatColor || '#F00';
             // Cap Shape
             ctx.fillStyle = hatCol;
             ctx.beginPath(); ctx.arc(p.x, headY - 5*s, headRadius * 1.1, Math.PI, 0); ctx.fill(); // Dome
             return;
        }

        // FALLBACK: Standard Short
        const r = modRadius * 1.02;
        ctx.fillStyle = hairColor;
        ctx.beginPath(); ctx.arc(p.x, headY - 2*s, r, 0, Math.PI*2); ctx.fill();
    }
