...
var BallRenderer = {
    ...
    function getCelColor(baseColor, type) {
        ...
        if (type === 'shadow') {
            hsl.l = Math.max(0, hsl.l - 20); // Darker
            hsl.s = Math.min(100, hsl.s + 10); // More saturated
            hsl.h = (hsl.h + 10) % 360; // Slight cool shift
        } else if (type === 'rim') {
            hsl.l = Math.min(100, hsl.l + 40); // Much lighter
    ...
    const drawFuzzyCircle = (cx, cy, r, c, seed = 50, scale = 1.0, furry = true, applyShading = true) => {
        if (playerData.graphics === 'HIGH') {
            ...
            if (furry) {
                ...
                ctx.stroke();
                // Shadow Fill
                drawFuzzyPath(g_circlePoints, shadowColor, scale, true, seed);
            } else {
                // Smooth Cel Circle
                ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI*2);
                ctx.strokeStyle = rimColor; ctx.lineWidth = 3*scale; ctx.stroke();
    ...
    function drawContinuousLimb(p1, p2, p3, w1, w2, w3, color, scale, isFurry, seed = 1, options = {}) {
        ...
        if (isFurry) {
            if (playerData.graphics === 'HIGH') {
                 ...
                 ctx.strokeStyle = getCelColor(color, 'rim');
                 ctx.lineWidth = 3*scale;
                 ctx.stroke();
                 // Fill
                 drawFuzzyPath(points, getCelColor(color, 'shadow'), scale, true, seed);
            } else {
    ...
    function drawFuzzyLimb(x1, y1, x2, y2, width, color, scale, isFurry, seed = 1, endWidth = null) {
        ...
        if (playerData.graphics === 'HIGH') {
            ...
            // Shadow Fill
            drawFuzzyPath(g_limbPoints, shadowColor, scale, true, seed);
        } else {
    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (isFurry) {
            ...
            if (playerData.graphics === 'HIGH') {
                ...
                // 2. Flat Shadow Fill
                ctx.fillStyle = getCelColor(color, 'shadow');
                ctx.fill();

                // 3. Subtle Spine Detail
                if (!options.isJersey) { // Only on skin
                    ctx.beginPath();
                    ctx.moveTo(cx, topY + h * 0.1);
                    ctx.lineTo(cx, topY + h * 0.8);
                    ctx.lineWidth = 2 * scale;
                    ctx.strokeStyle = 'rgba(0,0,0,0.15)'; // Very subtle dark line
                    ctx.stroke();
                }
            } else {
                // LEGACY / LOW GRAPHICS
