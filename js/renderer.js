var BallRenderer = {
    ...
    function drawHairstyle(ctx, p, headY, headRadius, s, skinObj) {
        ...
        if (style === 'flat_top') {
             ...
             ctx.lineTo(p.x + w*0.7, headY + 4*s); // Taper in at neck
             // Neck
             ctx.quadraticCurveTo(p.x, headY + 6*s, p.x - w*0.7, headY + 4*s);
             ctx.lineTo(p.x - w*0.9, headY - 2*s);
             ctx.lineTo(p.x - w*0.8, topY);
             ctx.fill();
             return;
    ...
    const PlayerRenderCache = {
        canvas: null,
        ctx: null,
        lastFrameTime: -1,
        data: { scale: 1, cx: 0, cy: 0 },
        ...
        get: function(obj, frameTime) {
            if (this.lastFrameTime !== frameTime) {
                this.generate(obj, frameTime);
            }
            return this.canvas;
        },

        generate: function(obj, frameTime) {
            this.init();
            this.lastFrameTime = frameTime;

            // Re-project player to get "canonical" appearance (main player at current depth)
            const proj = project(player3D.x, player3D.y, player3D.z, g_camCache);
            if (!proj) return; // Should not happen if player is visible

            const scale = proj.scale;
            // Ensure canvas is large enough
            const w = Math.ceil(250 * scale);
            const h = Math.ceil(400 * scale);

            if (this.canvas.width < w || this.canvas.height < h) {
                this.canvas.width = Math.max(this.canvas.width, w);
                this.canvas.height = Math.max(this.canvas.height, h);
                // Context is lost on resize? No, usually preserved or we re-get it.
                // But state might be lost. We only use default state anyway.
            }
            ...
            const cy = this.canvas.height * 0.8; // Draw feet near bottom
            ...
            const proxy = {
                ...
                y: cy,
                scale: scale,
                type: 'player', // Force 'player' type to enable foreshortening
                depth: obj.depth
            };
    ...
    function drawPlayer(p) {
        ...
        // Use cache only for High/Medium settings to optimize double-render (shadow)
        // Low graphics doesn't use realistic shadow, so no double render issue.
        // But we can still cache to save redraws if we wanted, but sticking to task scope.
        if (playerData.graphics === 'LOW') {
             _drawPlayerImpl(p);
             return;
        }

        const cacheCanvas = PlayerRenderCache.get(p, g_frameCounter);
        const data = PlayerRenderCache.data;

        // Destination: p.x, p.y correspond to data.cx, data.cy
        // Scale ratio: p.scale / data.scale
        const ratio = p.scale / data.scale;

        // Source Rect (entire active canvas area?)
        // We cleared the whole canvas, so we can draw the whole thing.
        // Optimally we'd track dirty rect, but full drawImage is fast.

        const dstW = cacheCanvas.width * ratio;
        const dstH = cacheCanvas.height * ratio;
        const dstX = p.x - (data.cx * ratio);
        const dstY = p.y - (data.cy * ratio);
