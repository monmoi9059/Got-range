...
var BallRenderer = {
    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (options.skinId && options.clothingType) {
            ...
            if (cStyle === 'baggy' || cType === 'hoodie' || cType === 'sweatshirt' || cType === 'track') {
                ctx.save();
                ctx.strokeStyle = 'rgba(0,0,0,0.15)'; ctx.lineWidth = 1.5*scale;

                // Armpit tension lines
                ctx.beginPath();
                ctx.moveTo(cx - w*0.9, topY + h*0.25);
                ctx.quadraticCurveTo(cx - w*0.7, topY + h*0.35, cx - w*0.6, topY + h*0.4);
                ctx.stroke();
                ...
                // Lower Back bunching
                ctx.beginPath();
                ctx.moveTo(cx - w*0.6, topY + h*0.85);
                ctx.bezierCurveTo(cx - w*0.2, topY + h*0.9, cx + w*0.2, topY + h*0.8, cx + w*0.6, topY + h*0.85);
                ctx.stroke();
                ...
                ctx.fillRect(cx - w, topY + h - 8*scale, w*2, 8*scale);
            }
    ...
    function drawShorts(cx, waistY, w, h, scale, skinObj) {
        if (skinObj.shortsType === 'kilt') {
             ...
             ctx.fillStyle = '#3E2723'; ctx.fillRect(cx - w/2, waistY, w, beltH);
             ctx.fillStyle = '#FFD700'; ctx.fillRect(cx - w*0.1, waistY, w*0.2, beltH);
        ...
        // Baggy Shorts (IMPROVED)
        const color = skinObj.shortsColor || '#FFF';
        const waistW = w * 1.0;
        const hipW = w * 1.3; // Flared hips for baggy look

        const bottomY = waistY + h;
        const crotchY = waistY + h * 0.45; // Higher crotch for basketball shorts
        ...
        // Crotch Area (V-cut)
        ctx.lineTo(cx + 2*scale, crotchY); // Inner Right Leg
        ctx.lineTo(cx - 2*scale, crotchY); // Inner Left Leg
        // Left Side
        ...
        if (skinObj.shortsPattern === 'bulls') {
            ctx.fillStyle = '#000'; // Trim
            ctx.beginPath(); ctx.moveTo(cx - hipW/2, bottomY - 15*scale); ctx.lineTo(cx - hipW/2 + 8*scale, bottomY - 8*scale); ctx.lineTo(cx - hipW/2, bottomY); ctx.fill();
            ctx.beginPath(); ctx.moveTo(cx + hipW/2, bottomY - 15*scale); ctx.lineTo(cx + hipW/2 - 8*scale, bottomY - 8*scale); ctx.lineTo(cx + hipW/2, bottomY); ctx.fill();
        }
        if(skinObj.sideStripesColor) {
            ctx.fillStyle = skinObj.sideStripesColor;
            const stripeW = 4 * scale;
            ctx.fillRect(cx - hipW/2, waistY, stripeW, h); // Left
            ctx.fillRect(cx + hipW/2 - stripeW, waistY, stripeW, h); // Right
        }

        // 3. Details (Pockets, Seams)
        ctx.strokeStyle = 'rgba(0,0,0,0.15)'; ctx.lineWidth = 1*scale;
        // Back Pockets (if back view - assumed yes for main player)
        ctx.beginPath(); ctx.rect(cx - waistW*0.35, waistY + 10*scale, waistW*0.25, waistW*0.25); ctx.stroke();
        ctx.beginPath(); ctx.rect(cx + waistW*0.1, waistY + 10*scale, waistW*0.25, waistW*0.25); ctx.stroke();

        // Center Seam
        ctx.beginPath(); ctx.moveTo(cx, waistY); ctx.lineTo(cx, crotchY); ctx.stroke();

        // Waistband (Elastic)
        ctx.fillStyle = 'rgba(0,0,0,0.05)';
        ctx.fillRect(cx - waistW/2, waistY, waistW, 5*scale);
        ctx.strokeStyle = 'rgba(0,0,0,0.1)';
        for(let i=0; i<waistW; i+=3*scale) {
             ctx.beginPath(); ctx.moveTo(cx - waistW/2 + i, waistY); ctx.lineTo(cx - waistW/2 + i, waistY + 5*scale); ctx.stroke();
        }

        // Folds at bottom (Baggy effect)
        ctx.strokeStyle = 'rgba(0,0,0,0.1)';
        ctx.lineWidth = 2*scale;
        ctx.beginPath(); ctx.moveTo(cx - hipW/2 + 5*scale, bottomY); ctx.lineTo(cx - hipW/2 + 5*scale, bottomY - 10*scale); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(cx + hipW/2 - 5*scale, bottomY); ctx.lineTo(cx + hipW/2 - 5*scale, bottomY - 10*scale); ctx.stroke();

        // Trim
        if (skinObj.trimColor) {
             ctx.fillStyle = skinObj.trimColor;
             ctx.fillRect(cx - hipW/2, bottomY - 3*scale, (hipW/2 - 2*scale), 3*scale); // Left
             ctx.fillRect(cx + 2*scale, bottomY - 3*scale, (hipW/2 - 2*scale), 3*scale); // Right
        }
    }
    ...
    function drawDetailedAccessory(ctx, type, cx, headY, headRadius, color, s) {
        const topY = headY - headRadius;
        ctx.fillStyle = color || '#333';

        if (type === 'cap' || type === 'hat_cap' || type === 'acc_cap') {
            const capColor = color || '#0047AB';
            ctx.fillStyle = capColor;
            // Dome
            const domeR = headRadius * 1.05;
            ctx.beginPath(); ctx.arc(cx, headY - 5*s, domeR, Math.PI, 0); ctx.fill();
            // Strap Gap
            ctx.fillStyle = '#1a1a1a';
            ctx.beginPath(); ctx.arc(cx, headY - 2*s, 3*s, Math.PI, 0); ctx.fill();
            // Button
            ctx.fillStyle = adjustColor(capColor, 20);
            ctx.beginPath(); ctx.arc(cx, headY - 5*s - domeR, 2*s, 0, Math.PI*2); ctx.fill();
            // Seams
            ctx.strokeStyle = 'rgba(0,0,0,0.3)'; ctx.lineWidth = 1*s;
            ctx.beginPath();
            ctx.moveTo(cx, headY - 5*s - domeR); ctx.lineTo(cx, headY - 5*s);
            ctx.moveTo(cx, headY - 5*s - domeR); ctx.lineTo(cx - domeR*0.7, headY - 5*s - domeR*0.7);
            ctx.moveTo(cx, headY - 5*s - domeR); ctx.lineTo(cx + domeR*0.7, headY - 5*s - domeR*0.7);
            ctx.stroke();

        } else if (type === 'top_hat' || type === 'hat_tophat' || type === 'acc_tophat') {
            const hatColor = color || '#111';
            ctx.fillStyle = hatColor;
            ctx.beginPath(); ctx.ellipse(cx, headY - 8*s, headRadius * 1.6, 5*s, 0, 0, Math.PI*2); ctx.fill();
            const cylW = headRadius * 1.5;
            const cylH = 25*s;
            const cylY = headY - 8*s - cylH;
            ctx.fillRect(cx - cylW/2, cylY, cylW, cylH);
            ctx.fillStyle = '#FF0000'; ctx.fillRect(cx - cylW/2, headY - 12*s, cylW, 4*s);
            ctx.fillStyle = adjustColor(hatColor, 10);
            ctx.beginPath(); ctx.ellipse(cx, cylY, cylW/2, 4*s, 0, Math.PI, 0); ctx.fill();

        } else if (type === 'cowboy' || type === 'hat_cowboy' || type === 'acc_cowboy') {
             const hatColor = color || '#8B4513';
             ctx.fillStyle = hatColor;
             ctx.beginPath();
             ctx.moveTo(cx - headRadius*1.8, headY - 10*s);
             ctx.quadraticCurveTo(cx, headY + 5*s, cx + headRadius*1.8, headY - 10*s);
             ctx.quadraticCurveTo(cx, headY - 15*s, cx - headRadius*1.8, headY - 10*s);
             ctx.fill();
             ctx.fillRect(cx - headRadius*0.7, headY - 25*s, headRadius*1.4, 15*s);
             ctx.strokeStyle = 'rgba(0,0,0,0.2)'; ctx.lineWidth=2*s;
             ctx.beginPath(); ctx.moveTo(cx, headY - 25*s); ctx.lineTo(cx, headY - 15*s); ctx.stroke();

        } else if (type === 'crown' || type === 'acc_crown') {
             ctx.fillStyle = '#FFD700'; ctx.strokeStyle = '#DAA520'; ctx.lineWidth = 1*s;
             const baseW = headRadius * 1.4;
             const baseY = headY - 10*s;
             ctx.beginPath(); ctx.ellipse(cx, baseY, baseW/2, 4*s, 0, 0, Math.PI*2); ctx.fill(); ctx.stroke();
             const pts = 5;
             ctx.beginPath(); ctx.moveTo(cx - baseW/2, baseY);
             for(let i=0; i<pts; i++) {
                 const x1 = cx - baseW/2 + (baseW/pts)*i;
                 const x2 = cx - baseW/2 + (baseW/pts)*(i+1);
                 const tipX = (x1+x2)/2;
                 const tipY = baseY - 15*s - (i===2 ? 5*s : 0);
                 ctx.lineTo(tipX, tipY); ctx.lineTo(x2, baseY);
             }
             ctx.fill(); ctx.stroke();
             ctx.fillStyle = '#FF0000'; ctx.beginPath(); ctx.arc(cx, baseY - 5*s, 3*s, 0, Math.PI*2); ctx.fill();
             ctx.fillStyle = '#00FF00'; ctx.beginPath(); ctx.arc(cx - baseW*0.3, baseY - 4*s, 2*s, 0, Math.PI*2); ctx.fill();
             ctx.fillStyle = '#0000FF'; ctx.beginPath(); ctx.arc(cx + baseW*0.3, baseY - 4*s, 2*s, 0, Math.PI*2); ctx.fill();

        } else if (type === 'halo') {
             ctx.strokeStyle = '#FFD700'; ctx.lineWidth = 3*s;
             ctx.shadowColor = '#FFD700'; ctx.shadowBlur = 10*s;
             ctx.beginPath(); ctx.ellipse(cx, headY - 25*s, headRadius, 4*s, 0, 0, Math.PI*2); ctx.stroke();
             ctx.shadowBlur = 0;

        } else if (type === 'headphones' || type === 'hat_headphones') {
             const bandColor = '#333';
             const cupColor = color || '#FFF';
             ctx.strokeStyle = bandColor; ctx.lineWidth = 4*s;
             ctx.beginPath(); ctx.arc(cx, headY - 5*s, headRadius + 4*s, Math.PI, 0); ctx.stroke();
             ctx.fillStyle = cupColor;
             ctx.beginPath(); ctx.ellipse(cx - headRadius - 4*s, headY, 5*s, 8*s, 0, 0, Math.PI*2); ctx.fill();
             ctx.beginPath(); ctx.ellipse(cx + headRadius + 4*s, headY, 5*s, 8*s, 0, 0, Math.PI*2); ctx.fill();

        } else {
             // Fallback
             ctx.fillStyle = color || '#555';
             ctx.fillRect(cx - headRadius, headY - 20*s, headRadius*2, 10*s);
        }
    }
    ...
    function drawRealisticHuman(p, s, skinObj) {
        ...
        // HELD BALL (Layer 0) - Draw before body if in lap (sitting) or behind
        const isDetachedStyle = ['airbud', 'telekinesis'].includes(playerData.currentStyle);
        if (!isDetachedStyle && state !== 'SHOOTING' && state !== 'GAMEOVER') {
             const isRightHand = !playerData.isLefty;
             const shootUAngle = isRightHand ? rightArmAngle : leftArmAngle;
        ...
        const drawHumanArm = (sx, sy, isRight, angle1, angle2, angle1_z, angle2_z) => {
            ...
            if (skinObj.clothingType === 'jacket') upperStartW *= 1.3;
            ...
            let armColor = skinTone;
            let sleeveColor = null;

            if (skinObj.clothingType === 'long_sleeve' || skinObj.clothingType === 'track' || skinObj.clothingType === 'hoodie' || skinObj.clothingType === 'sweatshirt' || skinObj.clothingType === 'jacket') {
                armColor = skinObj.jerseyColor;
            } else if (skinObj.clothingType === 'tshirt' || skinObj.jerseyType === 'tshirt') {
                // Short sleeve
                sleeveColor = skinObj.sleeveColor || skinObj.jerseyColor;
            } else if (skinObj.sleeveRight && isRight) {
                // Sleeve accessory
                sleeveColor = skinObj.sleeveRight;
            } else if (skinObj.sleeveLeft && !isRight) {
                sleeveColor = skinObj.sleeveLeft;
            }

            // Draw
            if (sleeveColor) {
                // Draw skin arm first
                drawContinuousLimb({x:sx,y:sy}, elbow, wrist, upperStartW, upperEndW, foreEndW, skinTone, s, false, 0);
                // Draw sleeve
                const sleeveLen = upperArmLen * uFactor * 0.45;
                const sleeveEnd = getJoint(sx, sy, sleeveLen, angle1);
                drawContinuousLimb({x:sx,y:sy}, sleeveEnd, sleeveEnd, upperStartW*1.1, upperStartW*1.0, upperStartW*1.0, sleeveColor, s, false, 0);
            } else {
                drawContinuousLimb({x:sx,y:sy}, elbow, wrist, upperStartW, upperEndW, foreEndW, armColor, s, false, 0);
            }

            // Hand
            ctx.save(); ctx.translate(wrist.x, wrist.y); ctx.rotate(angle2 + (isRight === !playerData.isLefty ? wristAngle : 0));
            ctx.fillStyle = skinTone;
            ctx.beginPath(); ctx.ellipse(0, 3*s, 3.5*s, 4.5*s, 0, 0, Math.PI*2); ctx.fill();
            // Thumb
            const thumbX = isRight ? -2.5*s : 2.5*s;
            ctx.beginPath(); ctx.ellipse(thumbX, 2*s, 1.5*s, 2.5*s, isRight ? 0.5 : -0.5, 0, Math.PI*2); ctx.fill();
            ctx.restore();
        };
        ...
        // Standard Stance
        lKneeX = p.x - 9*s; lKneeY = baseKneeY;
        rKneeX = p.x + 9*s; rKneeY = baseKneeY;
        lFootX = p.x - 10*s; lFootY = p.y;
        rFootX = p.x + 10*s; rFootY = p.y;
        ...
        const hipX = 7 * s * sizeMod.w;
        const thighStartW = 8 * s * sizeMod.legWidth;
        const thighEndW = thighStartW * 0.8;
        const calfEndW = thighEndW * 0.8;

        const drawHumanLeg = (hipP, kneeP, ankleP, isRight) => {
            let legColor = skinTone;
            if (skinObj.legType === 'pants' || skinObj.legType === 'tights') legColor = skinObj.shortsColor;
            ...
            if (skinObj.socksColor) {
                 const sockH = 0.4;
                 const sockTop = { x: kneeP.x + (ankleP.x-kneeP.x)*(1-sockH), y: kneeP.y + (ankleP.y-kneeP.y)*(1-sockH) };
                 drawLimb(sockTop.x, sockTop.y, ankleP.x, ankleP.y, calfEndW*1.1, skinObj.socksColor);
            }
            // Shoes
            if (skinObj.shoesColor) {
                 ...
                 ctx.beginPath(); ctx.moveTo(ankleP.x, ankleP.y - 2*s);
                 ctx.quadraticCurveTo(toeX, ankleP.y, toeX, ankleP.y + 3*s);
                 ctx.lineTo(ankleP.x, ankleP.y + 4*s); ctx.fill();
            }
        };
        ...
        ctx.fillStyle = skinTone;
        ctx.beginPath();
        ctx.moveTo(p.x - neckW*0.6, headY + headRadius*0.5); // Top (inside head)
        ctx.lineTo(p.x + neckW*0.6, headY + headRadius*0.5);
        ctx.lineTo(p.x + neckW*0.8, torsoY + 5*s); // Shoulders
        ctx.lineTo(p.x - neckW*0.8, torsoY + 5*s);
        ctx.fill();
        // Spine shadow
        ctx.fillStyle = 'rgba(0,0,0,0.1)';
        ...
        ctx.fillStyle = skinTone;
        const earY = headY + headRadius * 0.1;
        // Left Ear
        ctx.beginPath(); ctx.ellipse(p.x - headRadius*0.9, earY, 3*s, 5*s, -0.2, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = 'rgba(0,0,0,0.1)'; ctx.beginPath(); ctx.ellipse(p.x - headRadius*0.9, earY, 2*s, 4*s, -0.2, 0, Math.PI*2); ctx.fill(); // Inner shadow
        // Right Ear
        ctx.fillStyle = skinTone;
        ctx.beginPath(); ctx.ellipse(p.x + headRadius*0.9, earY, 3*s, 5*s, 0.2, 0, Math.PI*2); ctx.fill();
        ctx.fillStyle = 'rgba(0,0,0,0.1)'; ctx.beginPath(); ctx.ellipse(p.x + headRadius*0.9, earY, 2*s, 4*s, 0.2, 0, Math.PI*2); ctx.fill(); // Inner shadow
        ...
        // Head Circle (Crown)
        drawFuzzyCircle(p.x, headY, headRadius, skinTone, 0, s, false, true);
        ...
        if (playerData.currentHat && playerData.currentHat !== 'hat_none') {
             ...
             if (hat && typeof drawDetailedAccessory === 'function') {
                 drawDetailedAccessory(ctx, hat.type, p.x, headY, headRadius, hat.color, s);
             } else if (hat) {
                 // Fallback
                 ctx.fillStyle = hat.color || '#000';
                 ctx.fillRect(p.x - 10*s, headY - 10*s, 20*s, 5*s);
             }
        }

        // Shot Meter (Layer 6)
        if (typeof drawMeterCommon === 'function') {
    ...
    var PlayerRenderer = {
        ...
        if (['dog', 'wolf', 'fox', 'bear', 'pig', 'cow', 'moose', 'rat', 'lion', 'tiger'].includes(currentAnimal)) {
            ...
            // Slightly darker shade for snout
            const snoutColor = adjustColor(furColor, -10);
            ...
            if (currentAnimal !== 'moose') {
                ...
                const noseColor = (currentAnimal === 'pig') ? '#FF69B4' : '#000';
                if (isFurry) {
             ...
             if (playerData.currentCatAccessory && playerData.currentCatAccessory !== 'acc_none') {
                 ...
                 if (acc) {
                     if (acc.type === 'glasses') {
                         ...
                         drawDetailedAccessory(ctx, acc.id, p.x, headY, headR, acc.color, s);
                     }
