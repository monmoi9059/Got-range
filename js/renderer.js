var BallRenderer = {
    ...
    function drawHairstyle(ctx, p, headY, headRadius, s, skinObj) {
        ...
        const drawSolidLayeredBase = (radiusMod, lengthMod, color, isBack = false) => {
        ...
        if (style === 'curtains') {
             ...
             // Left Curtain
             ctx.beginPath();
             ctx.moveTo(p.x - 2*s, headY - r);
             ctx.bezierCurveTo(p.x - r*1.2, headY - r, p.x - r, headY + len, p.x - r*0.5, headY + len);
             ctx.lineTo(p.x - 2*s, headY + len*0.5);
             ctx.closePath();
             ctx.fill();
        ...
        if (style === 'pompadour') {
             ...
             ctx.moveTo(p.x - r, headY);
             // Bulge top
             ctx.bezierCurveTo(p.x - r, headY - h, p.x + r, headY - h, p.x + r, headY);
             ctx.lineTo(p.x + r, headY + 5*s);
             ctx.lineTo(p.x - r, headY + 5*s);
             ctx.fill();
             return;
