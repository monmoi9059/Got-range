...
var BallRenderer = {
    ...
    function drawHairstyle(ctx, p, headY, headRadius, s, skinObj) {
        ...
        // --- CUSTOM HAIR (Pixel Art from Creator) ---
        if (style.startsWith('custom_')) {
            // Find in playerData
            if (playerData.customHairstyles) {
                const customData = playerData.customHairstyles.find(h => h && h.id === style);
                if (customData) {
                    if (customData.blobs && customData.blobs.back) {
        ...
        if (style === 'bald_stubble') {
             ...
             ctx.beginPath(); ctx.arc(p.x, headY - 2*s, headRadius, 0, Math.PI*2); ctx.fill();
             ctx.globalAlpha = 1.0;
             // Stipple dots (Fixed seed for stability)
             ctx.fillStyle = adjustColor(hairColor, -20);
             const stubbleSeed = 12345;
             for(let i=0; i<30; i++) {
                 // Simple Pseudo Random
                 const r1 = Math.abs(Math.sin(stubbleSeed + i)) * Math.PI * 2;
                 const r2 = Math.abs(Math.cos(stubbleSeed + i * 1.5)) * headRadius * 0.9;

                 const angle = r1;
                 const dist = r2;
                 ctx.fillRect(p.x + Math.cos(angle)*dist, headY - 2*s + Math.sin(angle)*dist, 1.5*s, 1.5*s);
             }
             return;
        }
        ...
        if (style === 'buzz_cut' || style === 'buzz_colored') {
             ...
             if (style === 'buzz_colored') {
                 ...
                 const numSpots = 5;
                 const spotSeed = 54321; // Fixed
                 for(let i=0; i<numSpots; i++) {
                     // Spread points on surface
                     const r1 = Math.abs(Math.sin(spotSeed + i * 13));
                     const r2 = Math.abs(Math.cos(spotSeed + i * 7));

                     const angle = (r1 * Math.PI) - Math.PI/2;
                     const dist = r2 * modRadius * 0.8;
                     const sx = p.x + Math.sin(angle) * dist;
                     const sy = headY - 2*s - Math.cos(angle) * dist * 0.6;

                     // Irregular patches
                     ctx.beginPath();
                     ctx.ellipse(sx, sy, 6*s, 4*s, r1*3, 0, Math.PI*2);
                     ctx.fill();
                 }
                 ctx.restore();
             } else {
        ...
        if (style.startsWith('fade_')) {
             ...
             const topGrad = ctx.createLinearGradient(p.x, topY, p.x, headY);
             topGrad.addColorStop(0, adjustColor(hairColor, 30));
             topGrad.addColorStop(1, hairColor);
             ...
             if (style === 'fade_box') {
                 ...
                 // Forward swooping volume
                 ctx.moveTo(p.x - w, headY - s);
                 ctx.bezierCurveTo(p.x - w, topY - 5*s, p.x + w, topY - 5*s, p.x + w, headY - s);
                 ctx.lineTo(p.x + w, headY + 2*s);
                 ctx.lineTo(p.x - w, headY + 2*s);
                 ctx.fill();
                 ...
                 for(let i=0; i<=bumps; i++) {
                     ...
                     const bx = p.x + Math.cos(a)*w;
                     const by = headY + Math.sin(a)*topH;
                     // Control point slightly higher/out
                     const ca = Math.PI + ((i+0.5)/bumps)*Math.PI;
                     const cx_c = p.x + Math.cos(ca)*(w*1.2);
                     const cy_c = headY + Math.sin(ca)*(topH*1.2);
                     if (i < bumps) ctx.quadraticCurveTo(cx_c, cy_c, p.x + Math.cos(a + Math.PI/bumps)*w, headY + Math.sin(a + Math.PI/bumps)*topH);
                 }
                 ctx.lineTo(p.x + w, headY + 2*s);
                 ctx.lineTo(p.x - w, headY + 2*s);
             } else {
                 // Standard High Top (Rounded box)
                 ctx.moveTo(p.x - w, headY - s);
                 ctx.quadraticCurveTo(p.x, topY, p.x + w, headY - s);
                 ctx.lineTo(p.x + w, headY + 2*s);
                 ctx.lineTo(p.x - w, headY + 2*s);
                 // Add Texture Lines
                 if (style === 'fade_chef') {
                     ...
                     for(let k=0; k<5; k++) {
                         ctx.beginPath();
                         ctx.moveTo(p.x - w + k*4*s, topY + 2*s);
                         ctx.quadraticCurveTo(p.x, topY - 2*s, p.x + w - k*4*s, topY + 2*s);
                         ctx.stroke();
                     }
                     return;
                 }
        ...
        // --- 3. CURLS / AFROS / TEXTURE ---
        if (style.includes('afro') || style.includes('curls') || style === 'shaggy_top') {
             // Volumetric Cluster
             const baseR = modRadius * (style === 'afro_mini' ? 1.1 : (style === 'afro_70s' ? 1.5 : 1.2));
             const volumeY = (style === 'shaggy_top') ? headY - 5*s : headY - 5*s;

             // Base Sphere with Gradient
             const grad = ctx.createRadialGradient(p.x - baseR*0.3, volumeY - baseR*0.3, baseR*0.2, p.x, volumeY, baseR);
             grad.addColorStop(0, adjustColor(hairColor, 30));
             grad.addColorStop(0.4, hairColor);
             grad.addColorStop(1, adjustColor(hairColor, -30));

             ctx.fillStyle = grad;
             ctx.beginPath();
             ctx.arc(p.x, volumeY, baseR, 0, Math.PI*2);
             ctx.fill();

             // Texture: Loops or clumps
             ctx.strokeStyle = adjustColor(hairColor, -20);
             ctx.lineWidth = 1.5*s;

             if (style.includes('curls') || style === 'shaggy_top') {
                 // Loose curls
                 for(let i=0; i<12; i++) {
                     const a = (i/12)*Math.PI*2;
                     const r = baseR * 0.8;
                     const cx = p.x + Math.cos(a)*r;
                     const cy = volumeY + Math.sin(a)*r;
                     ctx.beginPath(); ctx.arc(cx, cy, 4*s, 0, Math.PI*2); ctx.stroke();
                 }
             } else {
                 // Tight Afro curls
                 for(let i=0; i<20; i++) {
                     const ang = (i * 137.5) * Math.PI/180; // Golden angle for even packing
                     const r = Math.sqrt(i) * baseR * 0.2;
                     const cx = p.x + Math.cos(ang)*r;
                     const cy = volumeY + Math.sin(ang)*r;
                     ctx.beginPath(); ctx.arc(cx, cy, 3*s, 0, Math.PI*2); ctx.stroke();
                 }
             }
             return;
        }
        ...
        if (style.includes('braids') || style.includes('cornrows') || style.includes('dreads')) {
             // Base scalp
             drawSolidLayeredBase(1.0, 1*s, adjustColor(hairColor, -10), true, 'natural');

             // Strands
             const isLong = style.includes('long') || style.includes('tied');
             const numStrands = style.includes('cornrows') ? 6 : 10;
             const width = headRadius * 1.8;
             const startX = p.x - width/2;
             const strandW = width / numStrands;

             ctx.lineCap = 'round';
             ctx.strokeStyle = hairColor;

             for(let i=0; i<=numStrands; i++) {
                 const x = startX + i*strandW;

                 // Gradient for strand depth
                 const strandGrad = ctx.createLinearGradient(x, headY - headRadius, x, headY + headRadius);
                 strandGrad.addColorStop(0, adjustColor(hairColor, 20));
                 strandGrad.addColorStop(0.5, hairColor);
                 strandGrad.addColorStop(1, adjustColor(hairColor, -20));
                 ctx.strokeStyle = strandGrad;
                 ctx.lineWidth = strandW * 0.8;

                 ctx.beginPath();
                 if (style.includes('cornrows')) {
                     // Tight to scalp
                     ctx.moveTo(x, headY - headRadius * 0.8);
                     ctx.lineTo(x, headY + headRadius * 0.8);
                 } else if (style.includes('zigzag')) {
                     // Wavy
                     ctx.moveTo(x, headY - headRadius * 0.8);
                     ctx.bezierCurveTo(x + 5*s, headY, x - 5*s, headY + 5*s, x, headY + headRadius);
                 } else {
                     // Dreads / Box Braids hanging
                     ctx.moveTo(x, headY);
                     const len = isLong ? headRadius * 2.5 : headRadius * 1.2;
                     // Gravity + Flow
                     const sway = Math.sin(seed + i) * 5*s;
                     ctx.quadraticCurveTo(x + sway, headY + len*0.5, x, headY + len);
                 }
                 ctx.stroke();

                 // Texture segments (Braiding)
                 if (style.includes('braids') || style.includes('cornrows')) {
                     ctx.strokeStyle = adjustColor(hairColor, -30);
                     ctx.lineWidth = 1*s;
                     const segLen = 4*s;
                     const len = isLong ? headRadius*2.5 : headRadius*1.6;
                     for(let y=0; y<len; y+=segLen) {
                         const py = headY - headRadius*0.8 + y;
                         ctx.beginPath(); ctx.moveTo(x - strandW*0.3, py); ctx.lineTo(x + strandW*0.3, py + 2*s); ctx.stroke();
                     }
                 }
             }
             ...
             if (style === 'dreads_tied') {
                 // Tie at back
                 ctx.fillStyle = skinObj.headbandColor || '#FFF';
                 ctx.fillRect(p.x - 10*s, headY + headRadius, 20*s, 4*s);
             }
             return;
        }

        // --- 5. LONG / FLOW / BOB / MULLET ---
        if (style === 'long_flow' || style === 'surfer_flow' || style === 'mullet_80s' || style.includes('shag') || style.includes('curtain')) {
             const len = (style === 'mullet_80s' || style === 'long_flow') ? headRadius * 2.8 : headRadius * 2.0;
             const isMullet = style === 'mullet_80s';

             // Base Gradient
             const grad = ctx.createLinearGradient(p.x, headY - headRadius, p.x, headY + len);
             grad.addColorStop(0, adjustColor(hairColor, 30));
             ...
             if (isMullet) {
                 // Short top, long back
                 ctx.arc(p.x, headY - 5*s, headRadius * 1.05, Math.PI, 0); // Top
                 ctx.lineTo(p.x + headRadius * 0.8, headY + len); // Back flow right
                 ctx.lineTo(p.x - headRadius * 0.8, headY + len); // Back flow left
                 ctx.lineTo(p.x - headRadius * 1.05, headY - 5*s);
             } else {
                 // Full flow
                 ctx.arc(p.x, headY, headRadius * 1.1, Math.PI, 0);
                 const spread = headRadius * 1.4;
                 ctx.bezierCurveTo(p.x + spread, headY + len*0.3, p.x + spread, headY + len, p.x + spread*0.8, headY + len);
                 ctx.lineTo(p.x - spread*0.8, headY + len);
                 ctx.bezierCurveTo(p.x - spread, headY + len, p.x - spread, headY + len*0.3, p.x - headRadius * 1.1, headY);
             }
             ctx.fill();
             return;
        }

        // --- 6. SPIKY / MOHAWK / TOP KNOT ---
        if (style === 'mohawk' || style === 'mohawk_short' || style === 'mohawk_fade') {
             // Fade sides
             drawSolidLayeredBase(1.0, 2*s, adjustColor(hairColor, -30), false, 'shaved');

             // Central strip
             const h = (style === 'mohawk') ? headRadius * 2.0 : headRadius * 1.0;
             const w = 12 * s;

             const grad = ctx.createLinearGradient(p.x, headY + 5*s, p.x, headY - h);
             grad.addColorStop(0, hairColor);
             grad.addColorStop(1, adjustColor(hairColor, 40)); // Lighter tip

             ctx.fillStyle = grad;
             ctx.beginPath();
             ctx.moveTo(p.x - w/2, headY + 5*s);
             ctx.quadraticCurveTo(p.x - w, headY - h/2, p.x, headY - h);
             ctx.quadraticCurveTo(p.x + w, headY - h/2, p.x + w/2, headY + 5*s);
             ctx.fill();
             return;
        }
        ...
        if (style === 'top_knot') {
             ...
             drawSolidLayeredBase(1.0, 2*s, adjustColor(hairColor, -30), false, 'shaved');
             // Bun
             const bunY = headY - headRadius * 0.8;
             const bunR = 10 * s;
             // Bun Gradient
             const bGrad = ctx.createRadialGradient(p.x - 2*s, bunY - 2*s, 2*s, p.x, bunY, bunR);
             bGrad.addColorStop(0, adjustColor(hairColor, 40));
             bGrad.addColorStop(1, hairColor);
        ...
        if (style === 'spiky' || style === 'ivy_league' || style === 'slick_side_part' || style === 'caesar_cut' || style === 'crew_messy') {
             // Standard textured top
             const topH = headRadius * 1.1;

             // Gradient
             const grad = ctx.createRadialGradient(p.x, headY - 5*s, 5*s, p.x, headY, headRadius * 1.2);
             grad.addColorStop(0, adjustColor(hairColor, 30));
             ...
             if (style === 'spiky') {
                 // Spikes geometry (Cones)
                 const spikes = 7;
                 const rO = headRadius * 1.6;
                 const rI = headRadius * 0.9;
                 for(let i=0; i<=spikes; i++) {
                     const a = Math.PI + (i/spikes)*Math.PI; // Top arc
                     const bx = p.x + Math.cos(a)*rI;
                     const by = headY + Math.sin(a)*rI;
                     const tx = p.x + Math.cos(a)*rO;
                     const ty = headY + Math.sin(a)*rO;
                     if(i===0) ctx.moveTo(bx, by);
                     ctx.lineTo(tx, ty);
                     // Mid point
                     if (i<spikes) {
                         const ma = Math.PI + ((i+0.5)/spikes)*Math.PI;
                         ctx.lineTo(p.x + Math.cos(ma)*rI, headY + Math.sin(ma)*rI);
                     }
                 }
                 ...
                 // Smooth top
                 ctx.arc(p.x, headY - 2*s, headRadius * 1.1, Math.PI, 0);
        ...
        // --- 7. DEFAULT FALLBACK ---
        drawSolidLayeredBase(1.1, 4*s, hairColor, true, 'natural');
    }
