...
var BallRenderer = {
    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (isFurry) {
            ...
            if (playerData.graphics === 'HIGH' && !isFurry) {
                 const animal = options.animal || 'human';
                 const isJersey = options.isJersey;

                 if (animal === 'human' || animal === 'monkey') {
                     // 6.0 BRAND NEW SHADING - UNIFIED (Jersey + Skin)

                     // 1. Base Gradient (Vertical Light) - Common
                     // Light top -> Dark bottom to simulate overhead stadium light
                     const baseGrad = ctx.createLinearGradient(0, topY, 0, topY + h);
                     baseGrad.addColorStop(0, 'rgba(255,255,255,0.15)'); // Stronger highlight on shoulders
                     baseGrad.addColorStop(0.4, 'rgba(0,0,0,0)');       // Mid back natural
                     baseGrad.addColorStop(0.8, 'rgba(0,0,0,0.2)');      // Lower back shadow
                     baseGrad.addColorStop(1, 'rgba(0,0,0,0.5)');        // Bottom hem/waist shadow
                     ...
                     if (isJersey) {
                         ...
                         // 2. Fabric Drapes (Vertical Folds)
                         // Subtle vertical highlights/shadows to simulate loose fabric hanging
                         const foldGrad = ctx.createLinearGradient(cx - w/2, 0, cx + w/2, 0);
                         foldGrad.addColorStop(0.0, 'rgba(0,0,0,0.3)'); // Side shadow
                         foldGrad.addColorStop(0.2, 'rgba(255,255,255,0.05)'); // Fold Highlight
                         foldGrad.addColorStop(0.4, 'rgba(0,0,0,0.1)'); // Fold Shadow
                         foldGrad.addColorStop(0.6, 'rgba(255,255,255,0.05)'); // Fold Highlight
                         foldGrad.addColorStop(0.8, 'rgba(0,0,0,0.1)'); // Fold Shadow
                         foldGrad.addColorStop(1.0, 'rgba(0,0,0,0.3)'); // Side shadow
                         ctx.fillStyle = foldGrad;
                         ctx.fillRect(cx - w*0.45, topY + h*0.2, w*0.9, h*0.8);

                         // 3. Tension Lines (From Armpits)
                         // Diagonal shadows indicating fabric stretching from shoulders
                         ctx.fillStyle = 'rgba(0,0,0,0.15)';
                         ctx.beginPath();
                         ctx.moveTo(cx - w*0.4, topY + h*0.2); // Left Armpit
                         ctx.lineTo(cx - w*0.1, topY + h*0.6); // Center Back
                         ctx.lineTo(cx - w*0.3, topY + h*0.6); // Wide
                         ctx.fill();

                         ctx.beginPath();
                         ctx.moveTo(cx + w*0.4, topY + h*0.2); // Right Armpit
                         ctx.lineTo(cx + w*0.1, topY + h*0.6); // Center Back
                         ctx.lineTo(cx + w*0.3, topY + h*0.6); // Wide
                         ctx.fill();

                         // 4. Bottom Hem Tuck / Swoop
                         // The fabric gathering at the waist
                         ctx.fillStyle = 'rgba(0,0,0,0.2)';
                         ctx.beginPath();
                         ctx.moveTo(cx - w/2, topY + h*0.85);
                         ctx.quadraticCurveTo(cx, topY + h*1.05, cx + w/2, topY + h*0.85);
                         ctx.lineTo(cx + w/2, topY + h);
                         ctx.lineTo(cx - w/2, topY + h);
                         ctx.fill();

                     } else {
                         // --- SKIN / MUSCLE SHADING (Shirtless) ---

                         // 2. Soft Spine Groove (Inverted Radial)
                         // A soft vertical shadow that fades out quickly width-wise
                         ctx.save();
                         ctx.beginPath();
                         ctx.ellipse(cx, topY + h*0.4, w*0.15, h*0.4, 0, 0, Math.PI*2);
                         ctx.clip();
                         ctx.fillStyle = 'rgba(0,0,0,0.15)';
                         ctx.fill();
                         ctx.restore();

                         // 3. Shoulder Blade Highlights (Soft Spots)
                         const drawBlade = (offsetX) => {
                             const bx = cx + offsetX;
                             const by = topY + h * 0.2;
                             const bGrad = ctx.createRadialGradient(bx, by, 0, bx, by, w*0.2);
                             bGrad.addColorStop(0, 'rgba(255,255,255,0.1)'); // Highlight
                             bGrad.addColorStop(1, 'rgba(0,0,0,0)');
                             ctx.fillStyle = bGrad;
                             ctx.beginPath(); ctx.arc(bx, by, w*0.2, 0, Math.PI*2); ctx.fill();
                         };
                         drawBlade(-w*0.25);
                         drawBlade(w*0.25);

                         // 4. Lat Shadows (Side Muscles)
                         // Darken the sides under the arms
                         const latGrad = ctx.createLinearGradient(cx - w/2, 0, cx + w/2, 0);
                         latGrad.addColorStop(0, 'rgba(0,0,0,0.2)');
                         latGrad.addColorStop(0.2, 'rgba(0,0,0,0)');
                         latGrad.addColorStop(0.8, 'rgba(0,0,0,0)');
                         latGrad.addColorStop(1, 'rgba(0,0,0,0.2)');
                         ctx.fillStyle = latGrad;
                         ctx.fillRect(cx - w/2, topY + h*0.2, w, h*0.6);
                     }
                 }
                 else {
                     // NON-HUMAN ANIMALS (Existing logic)
                     if (!isJersey) {
                         // Universal Spine Shadow
                         const spineGrad = ctx.createLinearGradient(cx - w*0.1, 0, cx + w*0.1, 0);
                         spineGrad.addColorStop(0, 'rgba(0,0,0,0)');
                         spineGrad.addColorStop(0.4, 'rgba(0,0,0,0.15)');
