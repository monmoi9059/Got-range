        ...
        if (isFurry) {
            ...
            if (!options.isJersey && playerData.graphics === 'HIGH' && !isFurry) {
                 ...
                 if (animal === 'dino') {
                     ...
                     // 6.0 BRAND NEW SHADING FROM SCRATCH
                     // Resetting to a high-contrast, stylized look to guarantee visibility of change.

                     // 1. Base Gradient (Vertical Light)
                     // Light top -> Dark bottom to simulate overhead stadium light
                     const baseGrad = ctx.createLinearGradient(0, topY, 0, topY + h);
                     baseGrad.addColorStop(0, 'rgba(255,255,255,0.1)');  // Shoulders lit
                     baseGrad.addColorStop(0.5, 'rgba(0,0,0,0)');       // Mid back natural
                     baseGrad.addColorStop(1, 'rgba(0,0,0,0.4)');       // Lower back shadow
                     ctx.fillStyle = baseGrad;
                     ctx.fillRect(cx - w/2, topY, w, h);

                     // 2. Soft Spine Groove (Inverted Radial)
                     // A soft vertical shadow that fades out quickly width-wise
                     const spineGrad = ctx.createRadialGradient(cx, topY + h*0.4, 0, cx, topY + h*0.4, w*0.3);
                     spineGrad.addColorStop(0, 'rgba(0,0,0,0.15)'); // Center darkness
                     spineGrad.addColorStop(1, 'rgba(0,0,0,0)');    // Fade to nothing

                     ctx.save();
                     ctx.scale(0.5, 1.0); // Squeeze horizontally to make it an oval/groove
                     ctx.translate(cx, 0); // Re-center after scale? No, simplified approach:
                     ctx.restore();

                     // Simpler approach for spine: Vertical linear gradient masked by an oval
                     ctx.save();
                     ctx.beginPath();
                     ...
                     // 4. Stylized "Drape" Shadow (The Swoop)
                     // Large, soft shadow swooping across the bottom to simulate a loose shirt
                     ctx.fillStyle = 'rgba(0,0,0,0.15)';
                     ctx.beginPath();
                     ctx.moveTo(cx - w/2, topY + h*0.7);
                     ctx.quadraticCurveTo(cx, topY + h*0.9, cx + w/2, topY + h*0.6);
                     ctx.lineTo(cx + w/2, topY + h);
                     ctx.lineTo(cx - w/2, topY + h);
                     ctx.fill();
                 }
