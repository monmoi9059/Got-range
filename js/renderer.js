...
var BallRenderer = {
    ...
    function getCelColor(baseColor, type) {
        ...
        if (type === 'shadow') {
            hsl.l = Math.max(0, hsl.l - 15); // Slightly Darker
            hsl.s = Math.min(100, hsl.s + 5); // Slightly More saturated
            hsl.h = (hsl.h + 5) % 360; // Slight cool shift
        } else if (type === 'rim') {
            hsl.l = Math.min(100, hsl.l + 15); // Subtle lightness boost (was +40)
            hsl.s = Math.max(0, hsl.s - 5); // Slight desaturate
    ...
    const drawFuzzyCircle = (cx, cy, r, c, seed = 50, scale = 1.0, furry = true, applyShading = true) => {
        if (playerData.graphics === 'HIGH') {
            ...
            // Volumetric Gradient (Backlit Sphere)
            const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
            grad.addColorStop(0.0, shadowColor); // Deep Shadow Center
            grad.addColorStop(0.6, baseColor);   // Mid-tone
            grad.addColorStop(1.0, rimColor);    // Rim Edge
            ...
            if (furry) {
                ...
                // Rim Stroke (Optional, keeps fuzzy edge defined)
                drawFuzzyPath(g_circlePoints, null, scale, true, seed, true);
                ctx.strokeStyle = rimColor;
                ...
                // Rim Stroke
                ctx.strokeStyle = rimColor; ctx.lineWidth = 2*scale; ctx.stroke();
                // Gradient Fill
    ...
    function drawContinuousLimb(p1, p2, p3, w1, w2, w3, color, scale, isFurry, seed = 1, options = {}) {
        ...
        if (isFurry) {
            if (playerData.graphics === 'HIGH') {
                 ...
                 // Gradient perpendicular to first segment (approx for whole limb)
                 const grad = ctx.createLinearGradient(p1L.x, p1L.y, p1R.x, p1R.y);
                 grad.addColorStop(0.0, rimColor);
            ...
            if (playerData.graphics === 'HIGH') {
                ...
                // Gradient perpendicular to first segment
                const grad = ctx.createLinearGradient(p1L.x, p1L.y, p1R.x, p1R.y);
                grad.addColorStop(0.0, rimColor);
                grad.addColorStop(0.15, baseColor);
                grad.addColorStop(0.5, shadowColor);
    ...
    function drawFuzzyLimb(x1, y1, x2, y2, width, color, scale, isFurry, seed = 1, endWidth = null) {
        ...
        if (playerData.graphics === 'HIGH') {
            ...
            // Gradient perpendicular to limb axis (p4 to p1)
            ...
            // Rim Stroke
            drawFuzzyPath(g_limbPoints, null, scale, true, seed, true);
            ctx.strokeStyle = rimColor;
            ctx.lineWidth = 2 * scale;
            ctx.lineCap = 'round';
            ctx.stroke();
    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (isFurry) {
            ...
            if (playerData.graphics === 'HIGH') {
                // VOLUMETRIC BACK VIEW (Backlit Torso)
                const rimColor = getCelColor(color, 'rim');
                const shadowColor = getCelColor(color, 'shadow');
                const baseColor = color;

                // Horizontal Gradient (Left -> Right)
                // Center is Darkest (Shadow), Edges are Lightest (Rim)
                const grad = ctx.createLinearGradient(cx - w, 0, cx + w, 0);
                grad.addColorStop(0.0, rimColor);   // Left Edge
                grad.addColorStop(0.2, baseColor);  // Turn
                grad.addColorStop(0.5, shadowColor);// Spine/Center (Deep Shadow)
                grad.addColorStop(0.8, baseColor);  // Turn
                grad.addColorStop(1.0, rimColor);   // Right Edge
    ...
    function drawLimb(x1, y1, x2, y2, width, color) {
        ...
        if (playerData.graphics === 'HIGH') {
            ...
            // Rim
            ctx.strokeStyle = rimColor;
            ctx.lineWidth = width * 0.2; // Thick rim relative to limb width? Or fixed?
            // Actually fixed might be better for consistent line weight
            // But limb width varies. Let's use scale based constant if we had 's'.
            // drawLimb doesn't take 's' explicitly, but 'width' is scaled.
            // Let's assume width * 0.2 is okay for rim or 3px.
