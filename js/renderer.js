...
var BallRenderer = {
    ...
    var g_cachedBallObj = null;
    var g_cachedBallId = null;
    var g_cachedMoneyBallObj = null;
    function drawBallSprite(x, y, scale, isFire, rotation, phys) {
        ...
        if(currentGameMode === 'CONTEST' && contestData.ballsInRack === 4) {
            if (!g_cachedMoneyBallObj) {
                g_cachedMoneyBallObj = BALLS_DB.find(function(b) { return b.id === 'ball_money'; }) || BALLS_DB[0];
            }
            ballObj = g_cachedMoneyBallObj;
        } else {
            var ballId = playerData.currentBall || 'ball_classic';
            if (ballId !== g_cachedBallId) {
                g_cachedBallId = ballId;
                g_cachedBallObj = BALLS_DB.find(function(b) { return b.id === ballId; }) || BALLS_DB[0];
            }
            ballObj = g_cachedBallObj;
        }
