...
var BallRenderer = {
    ...
    function drawHairStrand(ctx, p1, p2, cp1, cp2, width, color) {
        ctx.beginPath();
    ...
    function drawHairVolume(ctx, x, y, r, color, scale, type='cloud', seed=0) {
        if (type === 'cloud') {
            ...
            for(let i=0; i<steps; i++) {
                ...
                // Add shading variation per puff
                const shade = (i%2===0) ? color : adjustColor(color, -10);
                drawFuzzyCircle(ox, oy, r*0.6, shade, seed + i*10, scale, true, true);
            }
            // Core
            drawFuzzyCircle(x, y, r*0.7, color, seed, scale, true, true);
        } else if (type === 'spiky') {
            // Spiky volume
            ctx.fillStyle = color;
            ctx.beginPath();
            ...
            for(let i=0; i<=numSpikes; i++) {
                ...
                const spikeLen = r + (seededRandom(seed+i) * r * 0.4);
                const px = x + Math.cos(angle) * spikeLen;
                const py = y + Math.sin(angle) * spikeLen * 0.8; // Flatten slightly
                if(i===0) ctx.moveTo(px, py);
                else ctx.lineTo(px, py);
            }
            ctx.fill();
    ...
    function drawHairstyle(ctx, p, headY, headRadius, s, skinObj) {
        ...
        const shadowColor = adjustColor(hairColor, -30);
        const highlightColor = adjustColor(hairColor, 30);
        ...
        if (style === 'bald_stubble') {
             // Dense Particle Stubble
             ctx.fillStyle = adjustColor(hairColor, -10);
             const dens = 200; // Much denser
             for(let i=0; i<dens; i++) {
                 const angle = seededRandom(seed + i) * Math.PI * 2;
                 // Bias towards edges for 3D look
                 const r = Math.pow(seededRandom(seed + i + 100), 0.5) * headRadius;
                 const x = p.x + Math.cos(angle)*r;
                 const y = (headY - 2*s) + Math.sin(angle)*r * 0.9;

                 // Dot size varies slightly
                 const size = (0.5 + seededRandom(seed+i+200)*0.5) * s;
                 ctx.globalAlpha = 0.6;
        ...
        // --- 2. SHORT / BUZZ / FADES ---
        if (style.startsWith('fade_') || style === 'buzz_cut' || style === 'crew_messy' || style === 'caesar_cut' || style === 'ivy_league' || style === 'buzz_colored') {
            const isHighFade = (style === 'fade_high' || style === 'fade_box' || style === 'fade_pompadour');

            // 1. Skin/Fade Gradient Base
            const gradHeight = isHighFade ? headRadius * 0.8 : headRadius * 1.2;
            const fadeGrad = ctx.createLinearGradient(0, headY - headRadius, 0, headY + headRadius);
            fadeGrad.addColorStop(0, baseColor); // Top is hair
            fadeGrad.addColorStop(isHighFade ? 0.3 : 0.6, baseColor);
            fadeGrad.addColorStop(1, 'rgba(0,0,0,0)'); // Fade to skin (transparent over head)

            ctx.fillStyle = fadeGrad;
            ctx.beginPath();
            ctx.arc(p.x, headY - 2*s, modRadius * 1.02, 0, Math.PI*2);
            ctx.fill();

            // 2. Texture on Top (Particles)
            if (style !== 'buzz_colored') { // Buzz colored handles its own
                ctx.fillStyle = highlightColor;
                const dens = 50;
                for(let i=0; i<dens; i++) {
                    const r = seededRandom(seed + i) * modRadius * 0.8;
                    const a = seededRandom(seed + i + 50) * Math.PI * 2;
                    const tx = p.x + Math.cos(a)*r;
                    const ty = headY - 5*s + Math.sin(a)*r*0.6;
                    if (ty < headY - headRadius * 0.2) { // Top only
                        ctx.globalAlpha = 0.3;
            ...
            // 3. Specific Style Geometry (Volume added on top)
            if (style === 'fade_box' || style === 'fade_pompadour' || style === 'fade_king') {
                const topH = (style === 'fade_box') ? modRadius * 0.6 : modRadius * 0.4;
                const w = modRadius * 0.9;

                // Draw Volumetric Top
                ctx.fillStyle = baseColor;
                ctx.beginPath();
                ctx.moveTo(p.x - w, headY - modRadius*0.5);
                // Boxy or Round Top
                if (style === 'fade_box') {
                    ctx.lineTo(p.x - w, headY - modRadius - topH);
                    ctx.lineTo(p.x + w, headY - modRadius - topH);
                    ctx.lineTo(p.x + w, headY - modRadius*0.5);
                } else {
                    ctx.bezierCurveTo(p.x - w, headY - modRadius - topH*2, p.x + w, headY - modRadius - topH*2, p.x + w, headY - modRadius*0.5);
                }
                ctx.fill();

                // Add texture to the volume
                ctx.fillStyle = shadowColor;
                for(let i=0; i<20; i++) {
                    const rx = (seededRandom(seed+i)-0.5) * w * 1.8;
                    const ry = (seededRandom(seed+i+50)) * topH;
                    ctx.fillRect(p.x + rx, headY - modRadius - topH + ry, 2*s, 2*s);
                }
            } else if (style === 'buzz_colored') {
                 // Rodman Pattern
                 ...
                 for(let i=0; i<numSpots; i++) {
                     ...
                     const dist = (seededRandom(seed + i * 7)) * modRadius * 0.7;
                     const sx = p.x + Math.sin(angle) * dist;
                     const sy = headY - 5*s - Math.cos(angle) * dist * 0.7;
                     ctx.beginPath();
                     ctx.ellipse(sx, sy, 8*s, 5*s, seededRandom(seed+i)*3, 0, Math.PI*2);
                     ctx.fill();
                 }
            }
            return;
        ...
        // --- 3. AFROS / CURLS (VOLUMETRIC CLOUDS) ---
        if (style.startsWith('afro_') || style === 'curls_textured') {
            let sizeMult = 1.3;
            if (style === 'afro_blowout') sizeMult = 1.7;
            if (style === 'afro_taper') sizeMult = 1.3;
            if (style === 'curls_textured') sizeMult = 1.15;

            const r = modRadius * sizeMult;
            const volY = headY - 5*s;

            // Draw Base Volume (Cloud)
            drawHairVolume(ctx, p.x, volY, r, baseColor, s, 'cloud', seed);

            // Add stray curls for realism
            const strays = 12;
            ...
            for(let i=0; i<strays; i++) {
                ...
                const cy = volY + Math.sin(ang) * dist;
                // Little loop
                ctx.beginPath();
        ...
        if (style.startsWith('dreads_') || style === 'med_twist') {
            // Scalp
            ctx.fillStyle = adjustColor(baseColor, -20);
            ctx.beginPath(); ctx.arc(p.x, headY - 2*s, modRadius*0.9, 0, Math.PI*2); ctx.fill();

            const isTied = (style === 'dreads_tied');
            const numDreads = 25;
            const length = isTied ? 10*s : 25*s;

            for(let i=0; i<numDreads; i++) {
                // Random position on head sphere
                const ang = seededRandom(seed + i) * Math.PI * 2;
                const r = Math.sqrt(seededRandom(seed + i + 50)) * modRadius * 0.9;
                const sx = p.x + Math.cos(ang) * r;
                const sy = headY - 5*s + Math.sin(ang) * r * 0.8;
                ...
                if (isTied) {
                    // Up towards bun
                    ex = p.x + (seededRandom(seed+i)-0.5)*10*s;
                    ey = headY - modRadius - 10*s;
                    cp1x = sx;
                    cp1y = sy - 10*s;
                } else {
                    // Down and messy
                    ex = sx + (seededRandom(seed+i)-0.5)*20*s;
                    ey = sy + length;
                    cp1x = sx + (seededRandom(seed+i+100)-0.5)*15*s;
                    cp1y = sy + length * 0.5;
                }
                ...
                ctx.shadowBlur = 2;
                drawHairStrand(ctx, {x:sx, y:sy}, {x:ex, y:ey}, {x:cp1x, y:cp1y}, null, 5*s, baseColor);
                ctx.shadowBlur = 0;

                // Highlight
                drawHairStrand(ctx, {x:sx, y:sy}, {x:ex, y:ey}, {x:cp1x, y:cp1y}, null, 2*s, highlightColor);
            }

            if (isTied) {
                // Draw Bun
                drawFuzzyCircle(p.x, headY - modRadius - 8*s, 12*s, baseColor, seed, s, true, true);
            }
            return;
        }

        // --- 6. LONG / FLOW / WAVY ---
        if (style === 'long_flow' || style === 'long_layered' || style === 'med_wavy' || style === 'med_bob') {
            const length = 25 * s;

            // 1. Back Layer (Darker, behind shoulders)
            const numStrands = 30;
            for(let i=0; i<numStrands; i++) {
                const t = i/numStrands;
                const angle = (t - 0.5) * 4.0; // Wide spread

                const sx = p.x + Math.sin(angle) * modRadius * 0.8;
                const sy = headY - 10*s;

                const ex = p.x + Math.sin(angle) * modRadius * 1.5; // Flare out
                const ey = headY + length;

                const cp1x = sx + (ex-sx)*0.5 + (seededRandom(seed+i)-0.5)*20*s;
                const cp1y = sy + length*0.5;

                drawHairStrand(ctx, {x:sx, y:sy}, {x:ex, y:ey}, {x:cp1x, y:cp1y}, null, 8*s, shadowColor);
            }

            // 2. Top/Front Layer (Lighter, main volume)
            // Cap
            drawSolidLayeredBase(1.05, 5*s, baseColor, true, 'natural');

            // Front Strands
            for(let i=0; i<20; i++) {
                const t = i/20;
                const angle = (t - 0.5) * 3.0;

                const sx = p.x + Math.sin(angle) * modRadius;
                const sy = headY - 5*s;

                const ex = p.x + Math.sin(angle) * modRadius * 1.2 + (seededRandom(seed+i)-0.5)*10*s;
                const ey = headY + length * 0.8;

                const cp1x = sx + (seededRandom(seed+i+50)-0.5)*10*s;
                const cp1y = sy + length*0.4;

                drawHairStrand(ctx, {x:sx, y:sy}, {x:ex, y:ey}, {x:cp1x, y:cp1y}, null, 6*s, baseColor);
                // Highlight
                drawHairStrand(ctx, {x:sx, y:sy}, {x:ex, y:ey}, {x:cp1x, y:cp1y}, null, 2*s, highlightColor);
            }
            return;
        ...
        // --- 7. SPECIAL STYLES (Mullet, Slick, etc) ---
        if (style === 'mullet_80s' || style === 'anchor_man_80s') {
            const isAnchor = (style === 'anchor_man_80s');
            const len = isAnchor ? 15*s : 22*s;

            // Back Volume (Business in front, party in back)
            drawHairVolume(ctx, p.x, headY + 5*s, modRadius * 1.2, shadowColor, s, 'cloud', seed);

            // Top/Side Structure
            drawSolidLayeredBase(1.1, 5*s, baseColor, false, 'natural');

            // Side Wings
            ctx.fillStyle = baseColor;
            ctx.beginPath();
            ctx.moveTo(p.x - modRadius, headY);
            ctx.quadraticCurveTo(p.x - modRadius*1.5, headY + 10*s, p.x - modRadius*1.2, headY + len);
            ctx.quadraticCurveTo(p.x, headY + len + 5*s, p.x + modRadius*1.2, headY + len);
            ctx.quadraticCurveTo(p.x + modRadius*1.5, headY + 10*s, p.x + modRadius, headY);
            ctx.fill();
        ...
        if (style === 'top_knot' || style === 'med_bun') {
            // Slicked Base
            drawSolidLayeredBase(1.0, 0, baseColor, false, 'tapered');
            // Texture lines
            ctx.strokeStyle = highlightColor;
            ctx.lineWidth = 1*s;
            ctx.beginPath();
            for(let i=0; i<8; i++) {
                const x = p.x - modRadius*0.5 + i*s*3;
                ctx.moveTo(x, headY);
                ctx.lineTo(p.x, headY - modRadius);
            }
            ctx.stroke();

            // The Knot/Bun
            const bunY = headY - modRadius * (style==='top_knot' ? 1.2 : 0.8);
            drawFuzzyCircle(p.x, bunY, 8*s, baseColor, seed, s, true, true);
            return;
        }

        // --- DEFAULT FALLBACK ---
        drawSolidLayeredBase(1.0, 2*s, baseColor, true, 'natural');
    }
