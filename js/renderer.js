...
var BallRenderer = {
    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (anchors && anchors.shoulders && anchors.hips) {
            ...
            if (fitType === 'baggy') {
                ...
                const bagS = sW * 1.05; // Slightly wider shoulders
                const bagW = wW * 1.25; // Moderately Wide waist
                const bagH = hW * 1.15; // Gathered but wide hem
                ...
                const boxW = sW * 1.05; // Square frame
    ...
    function drawJersey(cx, topY, w, h, scale, skinObj, anchors = null) {
        ...
        if (fitType === 'baggy') {
            ...
            const bulgeW = (srX - cx) * 1.25; // Wider than shoulders at waist
            const hemW = (brX - cx) * 1.15;

            // Shoulders (Sloped down)
            ctx.moveTo(slX - shoulderW*0.05, slopeY); // Top Left (Out and Down)
            ctx.quadraticCurveTo(cx, sY - h*0.05, srX + shoulderW*0.05, slopeY); // Neck arc
            ...
            // Left Side
            ctx.bezierCurveTo(cx - hemW, bY, cx - bulgeW, sY + h*0.5, slX - shoulderW*0.05, slopeY);
        }
        else if (fitType === 'boxy') {
            // Rectangle: Broad shoulders, straight sides
            const boxS = (srX - cx) * 1.05; // Wider shoulders
            ...
            const fitS = (srX - cx) * 1.05;
            const fitW = (brX - cx) * 0.95; // Snatched waist
