    ...
    function drawAnatomicBody(cx, topY, w, h, scale, color, isFurry, seed = 1, options = {}, anchors = null) {
        ...
        if (options.skinId && options.clothingType) {
            ...
            if (cStyle === 'baggy' || cType === 'hoodie' || cType === 'sweatshirt') {
                ...
                // Draw Hem Band (Curved for round animals)
                ctx.beginPath();
                const hemH = 8 * scale;
                const hemY = topY + h - hemH;

                // Top edge of hem
                ctx.moveTo(cx - clothingW, hemY);
                // Gentle curve down to center
                ctx.quadraticCurveTo(cx, hemY + 5*scale, cx + clothingW, hemY);

                // Side down
                ctx.lineTo(cx + clothingW, hemY + hemH);

                // Bottom edge of hem
                ctx.quadraticCurveTo(cx, hemY + hemH + 5*scale, cx - clothingW, hemY + hemH);
