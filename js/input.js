    const GamepadController = {
        ...
        findBestFocusCandidate: function(dx, dy) {
            // Safety: If focused element is detached (dead), reset focus
            if (this.focusedElement && !document.body.contains(this.focusedElement)) {
                this.refreshFocusList(); // Will reset to index 0
            ...
            this.focusableElements.forEach(el => {
                ...
                // Direction Check (Relaxed tolerance to 10px)
                if (dx > 0 && c.x <= center.x + 10) return; // Right
                if (dx < 0 && c.x >= center.x - 10) return; // Left
                if (dy > 0 && c.y <= center.y + 10) return; // Down
                if (dy < 0 && c.y >= center.y - 10) return; // Up

                // Score = Distance + Alignment Penalty
                const distSq = Math.pow(c.x - center.x, 2) + Math.pow(c.y - center.y, 2);

                // Alignment Penalty (Reduced factor from 3.0 to 2.0)
                let penalty = 0;
                if (dy !== 0) penalty = Math.abs(c.x - center.x) * 2.0;
                if (dx !== 0) penalty = Math.abs(c.y - center.y) * 2.0;
        ...
        refreshFocusList: function() {
            ...
            // If focused element is lost or invalid, reset
    ...
    const deactivateGamepad = (e) => {
        // Prevent drift deactivation
        if (e.type === 'mousemove') {
            if (Math.abs(e.clientX - lastMouseX) < 10 && Math.abs(e.clientY - lastMouseY) < 10) return;
