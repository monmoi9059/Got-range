...
var BallRenderer = {
    ...
    function getScaleObject(dist) {
        ...
        _lastScaleDist = dist;
        let l = 0, r = SCALE_OBJECTS.length - 1;
        while (l <= r) {
            let m = (l + r) >>> 1;
            if (dist < SCALE_OBJECTS[m].limit) {
                if (m === 0 || dist >= SCALE_OBJECTS[m - 1].limit) {
                    _lastScaleObj = SCALE_OBJECTS[m];
    ...
    function getCourtDetails(dist) {
        ...
        _lastCourtDist = dist;
        let l = 0, r = COURT_ZONES.length - 1;
        while (l <= r) {
            let m = (l + r) >>> 1;
            if (dist < COURT_ZONES[m].limit) {
                if (m === 0 || dist >= COURT_ZONES[m - 1].limit) {
                    _lastCourtObj = COURT_ZONES[m];
