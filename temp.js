    ...
    var SKINS_DB = [
        ...
        { id: 'human_ai', animal: 'human', name: 'The Answer', cost: 35000, heightScale: 1.040, widthScale: 0.8, jerseyColor: '#000', shortsColor: '#000', trimColor: '#ED174C', number: '3', numberColor: '#FFF', skinTone: '#5c3a21', hairStyle: 'cornrows', hairColor: '#000', headbandColor: '#FFF', sleeveRight: '#000', socksColor: '#000', shoesColor: '#FFF', tattoos: true },
        ...
        { id: 'human_rodman', animal: 'human', name: 'The Worm', cost: 30000, heightScale: 1.140, widthScale: 0.9, jerseyColor: '#CE1141', shortsColor: '#000', trimColor: '#000', number: '91', numberColor: '#000', skinTone: '#3e271a', hairStyle: 'short', hairColor: '#00FF00', socksColor: '#FFF', shoesColor: '#FFF', shortsLength: 'short', tattoos: true },
        ...
        { id: 'human_hulk', animal: 'human', name: 'Géant Vert', cost: 20000, heightScale: 1.5, widthScale: 1.8, jerseyType: 'none', shortsColor: '#800080', skinTone: '#32CD32', hairStyle: 'short', hairColor: '#000', shoesColor: '#32CD32' },
    ...
    var BallRenderer = {
        draw: function(ctx, x, y, scale, rotation, ball) {
            var radius = 25 * scale;
        ...
        drawPattern: function(ctx, r, rot, ball) {
            ...
            if (type === 'basketball') {
                ...
                for(var i=0; i<Geometry.basketball.length; i++) {
                     ...
                     var proj = project(Geometry.basketball[i]);
                ...
                for(var i=0; i<Geometry.seam.length; i++) {
                    var proj = project(Geometry.seam[i]);
                    if (proj.z > -0.5) {
                        if (first) { ctx.moveTo(proj.x, proj.y); first=false; }
                        else ctx.lineTo(proj.x, proj.y);
                    } else {
                ...
                for(var i=0; i<Geometry.dimples.length; i++) {
                    var proj = project(Geometry.dimples[i]);
                    if (proj.z > 0) {
                        ctx.beginPath(); ctx.arc(proj.x, proj.y, r * 0.06, 0, Math.PI*2); ctx.fill();
                    }
                }
            }
            else if (type === 'bille8') {
                var proj = project({x:0, y:0, z:1});
                if (proj.z > 0) {
                    ctx.fillStyle = '#FFF';
                    ctx.beginPath(); ctx.arc(proj.x, proj.y, r * 0.45, 0, Math.PI*2); ctx.fill();
                    ctx.fillStyle = '#000';
                 ...
                 holes.forEach(function(h) {
                     var proj = project(h);
                     if (proj.z > 0) {
                         ctx.beginPath(); ctx.arc(proj.x, proj.y, r * 0.12, 0, Math.PI*2); ctx.fill();
                     }
                 });
            }
                ...
                if (proj.z > 0) {
                    ctx.fillStyle = ball.color2;
                    ctx.beginPath(); ctx.arc(proj.x, proj.y, r * 0.5, 0, Math.PI*2); ctx.fill();
                    ctx.fillStyle = '#000';
                    ctx.beginPath(); ctx.arc(proj.x, proj.y, r * 0.25, 0, Math.PI*2); ctx.fill();
                    ctx.fillStyle = 'rgba(255,255,255,0.8)';
                    ctx.beginPath(); ctx.arc(proj.x - r*0.15, proj.y - r*0.15, r * 0.12, 0, Math.PI*2); ctx.fill();
                }
            }
            else if (type === 'beach') {
                ...
                ctx.beginPath(); ctx.arc(0, -r*0.9, r*0.2, 0, Math.PI*2); ctx.fill();
                 ...
                 var icingCenter = project({x:0, y:-0.9, z:0});
                 if (icingCenter.z > -r) {
                     ctx.fillStyle = ball.color1;
                     ctx.beginPath(); ctx.arc(icingCenter.x, icingCenter.y, r * 0.85, 0, Math.PI*2); ctx.fill();
                     // Sprinkles
                     var sprinkleColors = ['#FFF', '#FF0', '#0FF'];
                     for(var i=0; i<8; i++) {
                         ctx.fillStyle = sprinkleColors[i%3];
                         ctx.fillRect(icingCenter.x + Math.cos(i)*r*0.5, icingCenter.y + Math.sin(i)*r*0.5, 4, 2);
                     }
                 }
                 ctx.fillStyle = '#222';
...
let lastDisplayedContestTime = -1;
    ...
    function closeLeaderboard() {
        ...
        if (feedback === "TERMINÉ !" || feedback === "MARCHÉ!" || feedback.includes("RECORD")) openShop();
    }
    ...
    function drawBallSprite(x, y, scale, isFire, rotation) {
        ...
        BallRenderer.draw(ctx, x, y, scale * 0.32, rotation, ballObj);
    ...
    function drawFuzzyLimb(x1, y1, x2, y2, width, color, scale, isFurry, seed = 1) {
        ...
        if (playerData.graphics === 'HIGH') {
             ...
             const grad3d = ctx.createLinearGradient(0, -w, 0, w);
             grad3d.addColorStop(0, 'rgba(0,0,0,0.3)');
             grad3d.addColorStop(0.5, 'rgba(255,255,255,0.1)'); // Highlight
             grad3d.addColorStop(1, 'rgba(0,0,0,0.3)');

             ctx.fillStyle = grad3d;
             ctx.beginPath();
             ctx.rect(0, -w, len, width);
             ctx.fill();
             ctx.restore();
        }
    ...
    function drawMuscleLimb(x1, y1, x2, y2, width, color, type, s, hasTattoos) {
        ...
        if (playerData.graphics === 'HIGH') {
            ...
            grad.addColorStop(0.4, 'rgba(255,255,255,0.25)'); // Highlight
    ...
    function drawJersey(cx, topY, w, h, scale, skinObj) {
        ...
        if (playerData.graphics === 'HIGH') {
             ...
             // Spine Indent
             ctx.beginPath();
             ctx.moveTo(cx, topY); ctx.lineTo(cx + 2*scale, bottomY); ctx.lineTo(cx - 2*scale, bottomY);
             ctx.fill();

             // Side Folds (Tension from armpits)
             ctx.beginPath();
             ctx.moveTo(cx - shoulderW/2, armpitY);
             ctx.quadraticCurveTo(cx - waistW*0.5, bottomY*0.8, cx - waistW/2 + 5*scale, bottomY);
             ctx.lineTo(cx - waistW/2, bottomY);
             ctx.fill();
    ...
    function drawLinkTunic(cx, topY, w, h, scale, skinObj) {
        ...
        const shoulderW = w * 1.6;
        const waistW = w * 1.0;
        const bottomW = w * 1.3; // Flared bottom

        const shoulderY = topY;
        const waistY = topY + h * 0.55;
        const bottomY = topY + tunicLen;
    ...
    function drawShorts(cx, waistY, w, h, scale, skinObj) {
        ...
        if (playerData.graphics === 'HIGH' && skinObj.shortsType !== 'kilt') {
             ...
             // Crotch radiated folds
             for(let i=0; i<3; i++) {
                 ctx.beginPath();
                 ctx.ellipse(cx, crotchY + h*0.1, h*0.4, h*0.1, (i-1)*0.5, 0, Math.PI*2);
                 ctx.fill();
             }

             // Side vertical folds
             ctx.beginPath();
             ctx.moveTo(cx - hipW/2 + 5*scale, waistY + h*0.5);
             ctx.lineTo(cx - hipW/2 + 10*scale, bottomY);
             ctx.lineTo(cx - hipW/2 + 2*scale, bottomY);
             ctx.fill();
    ...
    function drawRealisticHuman(p, s, skinObj) {
        ...
        if (skinObj.jerseyType === 'none') {
             // Bare Torso
             drawAnatomicBody(p.x, torsoY, reducedBodyW, jerseyH, s, skinTone, false, 0, { chestStripeColor: null, pattern: skinObj.pattern });
             ...
             drawLinkTunic(p.x, torsoY, reducedBodyW, jerseyH, s, skinObj);
        } else {
        ...
        if(skinObj.hairStyle === 'afro') {
             ...
             if (playerData.graphics === 'HIGH') {
                 ctx.fillStyle = 'rgba(0,0,0,0.2)';
                 for(let i=0; i<30; i++) {
                     ctx.fillRect(p.x + (Math.random()-0.5)*headRadius*1.8, headY - 5*s + (Math.random()-0.5)*15*s, 1.5*s, 1.5*s);
                 }
             }
        }
    ...
    function drawPlayer(p) {
        ...
        // Pose Logic
        const isSitting = (state === 'IDLE' && currentAnimal !== 'human');
        ...
        if (!bgCache || bgCache.distanceLevel !== distanceLevel || bgCache.mode !== currentGameMode) {
            ...
            if (playerData.graphics === 'HIGH') {
                 ...
                 if (court.type === 'arena') {
                     ...
                     // Random grain
                     fCtx.fillStyle = 'rgba(100, 50, 0, 0.05)';
                     for(let i=0; i<fCv.width*fCv.height*0.1; i++) {
                         fCtx.fillRect(Math.random()*fCv.width, Math.random()*fCv.height, 2, 1);
                     }
                 }
    ...
    function handleMiss(b) {
        ...
        if(currentGameMode === 'CONTEST') {
            feedback = "Manqué"; feedbackTimer = 30; state = 'RESETTING'; setTimeout(nextLevel, 500);
        } else if (currentGameMode === 'TIME_ATTACK') {
            ...
            if (consecutiveMisses >= maxMisses) {
                feedback = "TERMINÉ !"; feedbackTimer = 60; state = 'GAMEOVER'; setTimeout(() => checkGameOverSequence('classic', 10 + (distanceLevel * 5)), 1500);
            } else {
                feedback = "DERNIÈRE CHANCE !"; feedbackTimer = 60; state = 'RESETTING'; setTimeout(retryShot, 1500);
            }
        }
    }
    ...
    function update(dt) {
        ...
        if (state === 'JUMPING') {
            if (isGroundedShot) {
                ...
                if (player3D.z <= 0) { player3D.z = 0; player3D.vz = 0; state = 'GAMEOVER'; feedback = "MARCHÉ!"; feedbackTimer = 60; setTimeout(openShop, 1500); }
            }
        }
        if (state === 'SHOOTING') {
    ...
    function endContest() {
        ...
        checkAchievements('contest'); saveData(); setTimeout(() => checkGameOverSequence('contest', contestData.score), 2000);
    }
    ...
    function endTimeAttack() {
        ...
        saveData();
        setTimeout(() => checkGameOverSequence('time_attack', timeAttackData.score), 3000);
    }
    ...
    window.attemptReset = function() {
        ...
        resetStage = 0; btn.innerText = "RÉINITIALISER PROGRESSION"; btn.style.background = "#8B0000"; feedback = "RESET!"; feedbackTimer = 60;
    }
    ...
    window.closeShop = function() { state = 'IDLE'; shopUI.style.display = 'none'; if(feedback === "TERMINÉ !" || feedback === "MARCHÉ!") resetGame(); }
    window.closeAchievements = function() { state = 'IDLE'; achUI.style.display = 'none'; if(feedback === "TERMINÉ !" || feedback === "MARCHÉ!") resetGame(); }
    window.closeStats = function() {
        state = 'IDLE'; statsUI.style.display = 'none'; resetStage = 0;
        const btn = document.getElementById('btnReset'); if(btn) { btn.innerText = "RÉINITIALISER PROGRESSION"; btn.style.background = "#8B0000"; }
        if(feedback === "TERMINÉ !" || feedback === "MARCHÉ!") resetGame();
    }
    ...
    window.addEventListener('keydown', (e) => {
        // UI & P1 checks - Load P1 context to ensure UI uses correct state
        loadContext(game1);

        if(state === 'SHOP') { if(e.code === 'Escape') { closeShop(); saveContext(game1); return; } }
        if(state === 'ACHIEVEMENTS') { if(e.code === 'Escape') { closeAchievements(); saveContext(game1); return; } }
        if(state === 'STATS') { if(e.code === 'Escape') { closeStats(); saveContext(game1); return; } }
        if(state === 'LEADERBOARD') { if(e.code === 'Escape') { closeLeaderboard(); saveContext(game1); return; } }

        if(e.code === 'KeyP') { openShop(); saveContext(game1); return; }
        if(e.code === 'KeyO') { openAchievements(); saveContext(game1); return; }
        if(e.code === 'KeyS') { openStats(); saveContext(game1); return; }
        if(e.code === 'KeyL') { openLeaderboard(); saveContext(game1); return; }
        ...
        if (isSplitscreen && e.code === 'Enter' && !enterPressed) {
            enterPressed = true;
            // Restore P1 context before handling P2? No, doGameAction handles it.
            // But we currently have game1 loaded from above.
            // doGameAction will load game2.
    ...
    function saveContext(ctxObj) {
        ...
        // Input state is handled by event listeners updating the context directly,
        // but we can sync back if we used globals.
    }
    ...
    function toggleSplitscreen() {
        ...
        if(isSplitscreen) {
            ...
            game2.playerData = JSON.parse(JSON.stringify(playerData)); // Clone P1 profile
            game2.playerData.tacos = 0; // Fresh score

            // Offset P2 slightly so they don't look identical
            game2.player3D.x = 600;
    ...
    function drawSplitscreenHUD() {
        ...
        // P1 Score (Left)
        ctx.fillStyle = "#FFD700";
        ctx.fillText("P1 STREAK: " + game1.currentStreak, w * 0.25, 60);

        // P2 Score (Right)
        ctx.fillStyle = "#FFD700";
        ctx.fillText("P2 STREAK: " + game2.currentStreak, w * 0.75, 60);

        // Exit Hint
        ctx.font = "bold 16px 'Roboto Condensed'";
        ctx.fillStyle = "rgba(255,255,255,0.5)";
        ctx.fillText("P1: SPACE | P2: ENTER", w * 0.5, 30);
